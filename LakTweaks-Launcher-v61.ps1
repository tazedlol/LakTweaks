...
,
  {"CategoryId":"input","Category":"INPUT / MOUSE","Id":"device_powersave_wmi_off","Title":"Disable Device Power Saving (WMI)","Description":"Uses MSPower_DeviceEnable to disable device-level power saving. Based on your Disable Device Powersaving.ps1.","Command":":: === Disable Device Power Saving WMI ===\npowershell -NoProfile -ExecutionPolicy Bypass -Command \"try { Get-WmiObject MSPower_DeviceEnable -Namespace root\\wmi | ForEach-Object { $_.enable = $false; $_.psbase.put() | Out-Null }; Write-Host '[OK] Device power saving disabled through WMI' } catch { Write-Host '[WARN] WMI device power saving skipped:' $_.Exception.Message }\"\necho [OK] Device power saving WMI pass finished"},
  {"CategoryId":"input","Category":"INPUT / MOUSE","Id":"usb_idle_power_saving_off","Title":"Disable USB Idle + Selective Suspend","Description":"Disables common USB/controller idle and selective suspend registry values from your USB idle scripts.","Command":":: === Disable USB Idle / Selective Suspend ===\nfor %%A in (EnhancedPowerManagementEnabled AllowIdleIrpInD3 EnableSelectiveSuspend DeviceSelectiveSuspended SelectiveSuspendEnabled SelectiveSuspendOn WaitWakeEnabled D3ColdSupported WdfDirectedPowerTransitionEnable EnableIdlePowerManagement IdleInWorkingState) do (\n  for /f \"delims=\" %%B in ('reg query \"HKLM\\SYSTEM\\CurrentControlSet\\Enum\" /s /f \"%%A\" ^| findstr /i \"HKEY\"') do reg add \"%%B\" /v \"%%A\" /t REG_DWORD /d 0 /f >nul 2>&1\n)\npowercfg -setacvalueindex scheme_current sub_usb USBSELECTIVE SUSPEND 0 >nul 2>&1\npowercfg -setdcvalueindex scheme_current sub_usb USBSELECTIVE SUSPEND 0 >nul 2>&1\npowercfg /setactive scheme_current >nul 2>&1\necho [OK] USB idle/selective suspend disabled"},
  {"CategoryId":"input","Category":"INPUT / MOUSE","Id":"usb_controller_msi_priority","Title":"USB Controller MSI + Thread Priority","Description":"Enables MSI mode for USB controllers and sets USBHUB3/USBXHCI thread priority.","Command":":: === USB Controller MSI + Priority ===\nfor /f %%U in ('wmic path Win32_USBController get PNPDeviceID ^| findstr /L \"PCI\\VEN_\"') do (\n  reg add \"HKLM\\SYSTEM\\CurrentControlSet\\Enum\\%%U\\Device Parameters\\Interrupt Management\\MessageSignaledInterruptProperties\" /v MSISupported /t REG_DWORD /d 1 /f >nul 2>&1\n)\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\usbxhci\\Parameters\" /v ThreadPriority /t REG_DWORD /d 31 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\USBHUB3\\Parameters\" /v ThreadPriority /t REG_DWORD /d 31 /f >nul 2>&1\necho [OK] USB controller MSI/thread priority applied"},
  {"CategoryId":"gpu","Category":"GPU / NVIDIA","Id":"nvidia_force_p0_state","Title":"NVIDIA Force P0 State","Description":"Sets DisableDynamicPstate for NVIDIA display class keys. Based on your Force P0-State script.","Command":":: === NVIDIA Force P0 State ===\nfor /f %%I in ('wmic path Win32_VideoController get PNPDeviceID ^| findstr /L \"PCI\\VEN_10DE\"') do (\n  for /f \"tokens=3\" %%A in ('reg query \"HKLM\\SYSTEM\\ControlSet001\\Enum\\%%I\" /v Driver 2^>nul ^| findstr /i Driver') do (\n    reg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\Class\\%%A\" /v DisableDynamicPstate /t REG_DWORD /d 1 /f >nul 2>&1\n  )\n)\necho [OK] NVIDIA P0 state registry value applied if NVIDIA driver key exists"},
  {"CategoryId":"gpu","Category":"GPU / NVIDIA","Id":"nvidia_smi_ecc_off","Title":"NVIDIA ECC Off (If Supported)","Description":"Runs nvidia-smi -e 0 only if nvidia-smi exists. Safe no-op on unsupported GPUs.","Command":":: === NVIDIA ECC Off If Supported ===\nif exist \"%ProgramFiles%\\NVIDIA Corporation\\NVSMI\\nvidia-smi.exe\" (\n  \"%ProgramFiles%\\NVIDIA Corporation\\NVSMI\\nvidia-smi.exe\" -e 0 >nul 2>&1\n  echo [OK] NVIDIA ECC command sent if supported\n) else (\n  echo [OK] nvidia-smi not found; ECC tweak skipped\n)"},
  {"CategoryId":"gpu","Category":"GPU / NVIDIA","Id":"nvidia_unrestricted_clock_policy","Title":"NVIDIA Unrestricted Clock Policy","Description":"Runs nvidia-smi -acp 0 if available. Safe no-op if unsupported or nvidia-smi missing.","Command":":: === NVIDIA Unrestricted Clock Policy ===\nif exist \"%ProgramFiles%\\NVIDIA Corporation\\NVSMI\\nvidia-smi.exe\" (\n  \"%ProgramFiles%\\NVIDIA Corporation\\NVSMI\\nvidia-smi.exe\" -acp 0 >nul 2>&1\n  echo [OK] NVIDIA clock permission command sent if supported\n) else (\n  echo [OK] nvidia-smi not found; clock policy tweak skipped\n)"},
  {"CategoryId":"gpu","Category":"GPU / NVIDIA","Id":"gpu_hags_gamemode_pack","Title":"HAGS + Game Mode Pack","Description":"Enables HAGS and Game Mode while keeping capture disabled.","Command":":: === HAGS + Game Mode Pack ===\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers\" /v HwSchMode /t REG_DWORD /d 2 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\GameBar\" /v AutoGameModeEnabled /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_Enabled /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\GameDVR\" /v AppCaptureEnabled /t REG_DWORD /d 0 /f >nul 2>&1\necho [OK] HAGS/Game Mode pack applied"},
  {"CategoryId":"gpu","Category":"GPU / NVIDIA","Id":"nvidia_telemetry_cleanup","Title":"NVIDIA Telemetry Cleanup","Description":"Disables common NVIDIA telemetry tasks/services when present. Missing items are ignored.","Command":":: === NVIDIA Telemetry Cleanup ===\nfor %%S in (NvTelemetryContainer NvContainerNetworkService) do (sc stop %%S >nul 2>&1 & sc config %%S start= disabled >nul 2>&1)\nfor %%T in (NvTmMon NvTmRep NvTmRepOnLogon) do schtasks /Change /TN \"\\NVIDIA\\%%T\" /Disable >nul 2>&1\nreg delete \"HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run\" /v NvBackend /f >nul 2>&1\necho [OK] NVIDIA telemetry cleanup completed"},
  {"CategoryId":"fortnite","Category":"FORTNITE FPS / PERFORMANCE","Id":"fse_safe_fullscreen_pack","Title":"FSE Safe Fullscreen Pack","Description":"Applies your FSE fullscreen values without fighting the GameDVR/FSE conflict fix.","Command":":: === FSE Safe Fullscreen Pack ===\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_FSEBehaviorMode /t REG_DWORD /d 2 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_FSEBehavior /t REG_DWORD /d 2 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_DSEBehavior /t REG_DWORD /d 2 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_DXGIHonorFSEWindowsCompatible /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_HonorUserFSEBehaviorMode /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_EFSEFeatureFlags /t REG_DWORD /d 0 /f >nul 2>&1\necho [OK] FSE safe fullscreen values applied"},
  {"CategoryId":"power","Category":"POWER PLAN","Id":"timer_power_gaming_safe","Title":"Timer Power Gaming Safe","Description":"Creates/enables Ultimate Performance and disables AC sleep/hibernate without permanent HPET/BCD flags.","Command":":: === Timer Power Gaming Safe ===\npowercfg -duplicatescheme e9a42b02-d5df-448d-aa00-03f14749eb61 >nul 2>&1\nfor /f \"tokens=3\" %%A in ('powercfg -l ^| findstr /i \"Ultimate\"') do powercfg -setactive %%A >nul 2>&1\npowercfg -x -monitor-timeout-ac 0 >nul 2>&1\npowercfg -x -standby-timeout-ac 0 >nul 2>&1\npowercfg -hibernate off >nul 2>&1\necho [OK] Timer/Power gaming safe preset applied"},
  {"CategoryId":"power","Category":"POWER PLAN","Id":"aggressive_power_plan_pack","Title":"Aggressive Power Plan Pack","Description":"Applies CPU 100%, boost policy, idle promote/demote, no sleep, USB/device idle off. Reboot recommended.","Command":":: === Aggressive Power Plan Pack ===\npowercfg -setacvalueindex scheme_current sub_processor PROCTHROTTLEMIN 100 >nul 2>&1\npowercfg -setacvalueindex scheme_current sub_processor PROCTHROTTLEMAX 100 >nul 2>&1\npowercfg -setdcvalueindex scheme_current sub_processor PROCTHROTTLEMIN 100 >nul 2>&1\npowercfg -setdcvalueindex scheme_current sub_processor PROCTHROTTLEMAX 100 >nul 2>&1\npowercfg -setacvalueindex scheme_current sub_processor CPMINCORES 100 >nul 2>&1\npowercfg -setacvalueindex scheme_current sub_processor PERFEPP 0 >nul 2>&1\npowercfg -setacvalueindex scheme_current sub_processor PERFBOOSTPOL 100 >nul 2>&1\npowercfg -setacvalueindex scheme_current SUB_SLEEP AWAYMODE 0 >nul 2>&1\npowercfg -setacvalueindex scheme_current SUB_SLEEP ALLOWSTANDBY 0 >nul 2>&1\npowercfg -setacvalueindex scheme_current SUB_SLEEP HYBRIDSLEEP 0 >nul 2>&1\npowercfg -setacvalueindex scheme_current sub_none DEVICEIDLE 0 >nul 2>&1\npowercfg /setactive scheme_current >nul 2>&1\necho [OK] Aggressive power plan pack applied"},
  {"CategoryId":"network","Category":"NETWORK OPTIMIZATION","Id":"wifi_services_repair","Title":"Wi-Fi Services Repair","Description":"Re-enables core Wi-Fi/network services and active probing if networking breaks after extreme debloat.","Command":":: === Wi-Fi Services Repair ===\nfor %%S in (Dhcp DPS lmhosts NlaSvc nsi RmSvc Wcmsvc WlanSvc EventLog) do sc config %%S start= auto >nul 2>&1\nfor %%S in (Netman WwanSvc WinHttpAutoProxySvc LanmanWorkstation) do sc config %%S start= demand >nul 2>&1\nreg add \"HKLM\\Software\\Policies\\Microsoft\\Windows\\NetworkConnectivityStatusIndicator\" /v NoActiveProbe /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\System\\CurrentControlSet\\Services\\NlaSvc\\Parameters\\Internet\" /v EnableActiveProbing /t REG_DWORD /d 1 /f >nul 2>&1\nipconfig /flushdns >nul 2>&1\necho [OK] Wi-Fi/network services repair applied"},
  {"CategoryId":"registry","Category":"REGISTRY TWEAKS","Id":"windows_latency_registry_pack","Title":"Windows Latency Registry Pack","Description":"Adds selected safer latency keys from your windows.reg/kbm pack. Avoids CSRSS and risky processor C-state keys.","Command":":: === Windows Latency Registry Pack ===\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\PriorityControl\" /v Win32PrioritySeparation /t REG_DWORD /d 26 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\services\\mouclass\\Parameters\" /v MouseDataQueueSize /t REG_DWORD /d 18 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games\" /v \"GPU Priority\" /t REG_DWORD /d 8 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games\" /v Priority /t REG_DWORD /d 6 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games\" /v \"Scheduling Category\" /t REG_SZ /d High /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games\" /v \"SFIO Priority\" /t REG_SZ /d High /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\Psched\" /v NonBestEffortLimit /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\Power\\PowerThrottling\" /v PowerThrottlingOff /t REG_DWORD /d 1 /f >nul 2>&1\necho [OK] Windows latency registry pack applied"}
...
function New-LakRestorePoint {
    try {
        Write-Log 'Creating restore point: LakTweaks v61 Before Apply...' 'INFO'
        Checkpoint-Computer -Description 'LakTweaks v61 Before Apply' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
...
function Backup-LakFullRegistry {
    try {
        ...
        [System.Windows.MessageBox]::Show('Full registry backup saved to:`n' + $dir, 'LakTweaks v61', 'OK', 'Information') | Out-Null
...
function Restore-LakFullRegistry {
    try {
        ...
        $r = [System.Windows.MessageBox]::Show('Restore the saved full registry backup now? You should reboot after restoring.', 'LakTweaks v61', 'YesNo', 'Warning')
        if ($r -ne 'Yes') { return }
...
function New-LakStateSnapshot([object[]]$selected) {
    try {
        ...
        [pscustomobject]@{ Version='v61 PRO+'; Time=(Get-Date).ToString('s'); Count=$selected.Count; Tweaks=@($selected | Select-Object Id,Title,CategoryId) } | ConvertTo-Json -Depth 5 | Set-Content -Path (Join-Path $dir 'session.json') -Encoding UTF8
        foreach($r in @(
...
function Restore-LakLastSnapshot {
    try {
        ...
        $r = [System.Windows.MessageBox]::Show('Restore last LakTweaks snapshot? Reboot after restoring.', 'LakTweaks v61', 'YesNo', 'Warning')
        if ($r -ne 'Yes') { return }
  ...
  <Grid>
      ...
      <Grid Margin="22,0,22,0">
        ...
        <StackPanel Orientation="Vertical" VerticalAlignment="Center">
          <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
            ...
            <TextBlock Text=" v61" FontSize="25" FontWeight="Black" Foreground="#00ffe7" Margin="14,0,0,0"/>
    ...
    <Grid x:Name="MainContentGrid" Grid.Row="1" Grid.Column="1" Margin="16,16,16,14">
      ...
      <Grid Margin="22,0">
        ...
        <Border Grid.Column="3" CornerRadius="10" BorderBrush="#00a7c8" BorderThickness="1" Padding="16,7" HorizontalAlignment="Center" VerticalAlignment="Center"><TextBlock Text="v61.0.0" Foreground="#00ffe7" FontSize="13" FontWeight="Black"/></Border>
      </Grid>
    </Border>
...
function Show-UiSettings {
    ...
    $aTitle.Text = 'LAK TWEAKS GOD  —  Version 61'
...
function Invoke-ApplySelected {
    ...
    try {
        ...
        if ($result.Failed.Count -gt 0) {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count), failed $($result.Failed.Count). Check the log, then use RETRY FAILED.", 'LakTweaks v61', 'OK', 'Warning') | Out-Null
        } else {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count) tweak(s) successfully.`nReboot your PC for full effect.", 'LakTweaks v61', 'OK', 'Information') | Out-Null
        }
    } catch {
...
(& $find 'ClearLogBtn').Add_Click({
    if ($null -ne $LogBox) { $LogBox.Text = 'LakTweaks v61 log cleared.' }
...
Write-Log 'v61 PRO+: undo snapshots, verify mode, progress engine, conflict guard, uploaded USB/GPU/power tweaks added.' 'OK'
Show-Section 'home'
Update-SelectedCount
