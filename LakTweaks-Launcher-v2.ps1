...
function New-UiSliderRow {
    param(
        ...
        [Parameter(Mandatory=$true)][scriptblock]$OnChange
    )

    $sp=[Windows.Controls.StackPanel]::new(); $sp.Margin='0,6,0,12'
    $row=[Windows.Controls.DockPanel]::new()
    $tb=[Windows.Controls.TextBlock]::new(); $tb.Text=$Label; $tb.Foreground='White'; $tb.FontWeight='SemiBold'; [Windows.Controls.DockPanel]::SetDock($tb,'Left'); $row.Children.Add($tb)|Out-Null
    $valText=[Windows.Controls.TextBlock]::new(); $valText.Text=[string][int][Math]::Round($Value); $valText.Foreground=Get-ColorBrush $script:CurrentAccent; $valText.HorizontalAlignment='Right'; [Windows.Controls.DockPanel]::SetDock($valText,'Right'); $row.Children.Add($valText)|Out-Null
    $sp.Children.Add($row)|Out-Null

    $sl=[Windows.Controls.Slider]::new(); $sl.Minimum=$Minimum; $sl.Maximum=$Maximum; $sl.Value=$Value; $sl.TickFrequency=1; $sl.IsSnapToTickEnabled=$false; $sl.IsMoveToPointEnabled=$true; $sl.Margin='0,6,0,0'; $sl.Height=24; $sl.Width=420
    $sl.Add_ValueChanged({
        param($sender,$args)
        $valText.Text = [string][int][Math]::Round($sender.Value)
...
function Show-UiSettings {
    ...
    $host=[Windows.Controls.Grid]::new()
    $host.Margin='0,0,0,14'
    $host.Width = [Math]::Max(920, [Math]::Min(1120, $Window.ActualWidth - 380))
    $host.ColumnDefinitions.Add([Windows.Controls.ColumnDefinition]::new())
    $host.ColumnDefinitions.Add([Windows.Controls.ColumnDefinition]::new())
    $host.ColumnDefinitions[0].Width = [Windows.GridLength]::new(1,[Windows.GridUnitType]::Star)
    $host.ColumnDefinitions[1].Width = [Windows.GridLength]::new(1,[Windows.GridUnitType]::Star)

    $left=[Windows.Controls.Border]::new(); $left.Margin='0,0,18,0'; $left.Padding='18'; $left.CornerRadius='12'; $left.Background=Get-ArgbBrush $script:CurrentAccent (0.04 + ($script:CurrentBgOpacity * 0.06)); $left.BorderBrush=Get-ArgbBrush $script:CurrentAccent (0.20 + ($script:CurrentGlow * 0.20)); $left.BorderThickness='1'
    ...
    $d=[Windows.Controls.TextBlock]::new(); $d.Text='Tune the whole app look here. These controls now update the UI correctly and the full options stay visible.'; $d.TextWrapping='Wrap'; $d.Foreground=Get-ColorBrush '#7ca7a2'; $d.Margin='0,6,0,14'; $stack.Children.Add($d)|Out-Null

    $presetPanel=[Windows.Controls.WrapPanel]::new(); $presetPanel.Margin='0,0,0,12'; $presetPanel.ItemWidth=110
    foreach($preset in @(@{Name='Cyan Core';Hex='#00ffe7'},@{Name='Pink Surge';Hex='#ff3cac'},@{Name='Gold Rush';Hex='#f9c74f'},@{Name='Purple Voltage';Hex='#784dff'},@{Name='Red Strike';Hex='#ff4757'})) {
        $b=[Windows.Controls.Button]::new(); $b.Content=$preset.Name; $b.Margin='0,0,8,8'; $b.Padding='10,10'; $b.MinWidth=100; $b.Background=Get-ArgbBrush $preset.Hex 0.18; $b.BorderBrush=Get-ArgbBrush $preset.Hex 0.55; $b.Foreground='White'; $b.Tag=$preset.Hex
        $b.Add_Click({
            param($sender,$args)
    ...
    $null = New-UiSliderRow -Parent $stack -Label 'Background Opacity' -Minimum 45 -Maximum 100 -Value ($script:CurrentBgOpacity*100) -OnChange {
        param($v)
        $script:CurrentBgOpacity = [math]::Round($v/100,2)
        Save-UiSettings
        Apply-Theme
    }
    $null = New-UiSliderRow -Parent $stack -Label 'Glow Strength' -Minimum 0 -Maximum 100 -Value ($script:CurrentGlow*100) -OnChange {
        param($v)
        $script:CurrentGlow = [math]::Round($v/100,2)
        Save-UiSettings
        Apply-Theme
    }
    $null = New-UiSliderRow -Parent $stack -Label 'Grid Strength' -Minimum 0 -Maximum 100 -Value ($script:CurrentGrid*100) -OnChange {
        param($v)
        $script:CurrentGrid = [math]::Round($v/100,2)
        Save-UiSettings
        Apply-Theme
    }
    $null = New-UiSliderRow -Parent $stack -Label 'Scanlines' -Minimum 0 -Maximum 100 -Value ($script:CurrentScanlines*100) -OnChange {
        param($v)
        $script:CurrentScanlines = [math]::Round($v/100,2)
        Save-UiSettings
        Apply-Theme
    }
    ...
    $compactCb=[Windows.Controls.CheckBox]::new(); $compactCb.Content='Compact Mode'; $compactCb.IsChecked=$script:CurrentCompact; $compactCb.Margin='0,10,0,6'; $compactCb.Foreground='White'; $compactCb.Add_Click({ param($sender,$args) $script:CurrentCompact = [bool]$sender.IsChecked; Save-UiSettings; if($SidebarList.SelectedItem -and $SidebarList.SelectedItem.Tag -eq 'ui'){ Show-UiSettings } elseif($SidebarList.SelectedItem){ Show-Section $SidebarList.SelectedItem.Tag } }); $stack.Children.Add($compactCb)|Out-Null
    $cleanCb=[Windows.Controls.CheckBox]::new(); $cleanCb.Content='Clean Mode'; $cleanCb.IsChecked=$script:CurrentClean; $cleanCb.Margin='0,0,0,10'; $cleanCb.Foreground='White'; $cleanCb.Add_Click({ param($sender,$args) $script:CurrentClean = [bool]$sender.IsChecked; Save-UiSettings; if($SidebarList.SelectedItem -and $SidebarList.SelectedItem.Tag -eq 'ui'){ Show-UiSettings } elseif($SidebarList.SelectedItem){ Show-Section $SidebarList.SelectedItem.Tag } }); $stack.Children.Add($cleanCb)|Out-Null
    ...
    $right=[Windows.Controls.Border]::new(); $right.Padding='18'; $right.CornerRadius='12'; $right.Background=Get-ArgbBrush '#0e171d' ([Math]::Max(0.78,$script:CurrentBgOpacity)); $right.BorderBrush=Get-ArgbBrush $script:CurrentAccent (0.20 + ($script:CurrentGlow * 0.20)); $right.BorderThickness='1'
    ...
    $rd=[Windows.Controls.TextBlock]::new(); $rd.Text='This updates as you change presets, opacity, glow, compact mode, and clean mode.'; $rd.TextWrapping='Wrap'; $rd.Foreground=Get-ColorBrush '#7ca7a2'; $rd.Margin='0,6,0,14'; $rstack.Children.Add($rd)|Out-Null
    ...
    $pcs=[Windows.Controls.StackPanel]::new(); $pct=[Windows.Controls.TextBlock]::new(); $pct.Text='Fortnite FPS Boost'; $pct.Foreground='White'; $pct.FontWeight='Bold'; $pcs.Children.Add($pct)|Out-Null; $pcd=[Windows.Controls.TextBlock]::new(); $pcd.Text='Neon accents, glass cards, responsive controls, and saved UI settings.'; $pcd.TextWrapping='Wrap'; $pcd.Margin='0,8,0,0'; $pcd.Foreground=Get-ColorBrush '#7ca7a2'; $pcs.Children.Add($pcd)|Out-Null; $pc.Child=$pcs; $pv.Children.Add($pc)|Out-Null

    $gridBar=[Windows.Controls.Border]::new(); $gridBar.Margin='0,14,0,0'; $gridBar.Padding='10'; $gridBar.Background=Get-ArgbBrush $script:CurrentAccent (0.03 + ($script:CurrentGrid * 0.12)); $gridBar.BorderBrush=Get-ArgbBrush $script:CurrentAccent (0.08 + ($script:CurrentGlow * 0.18)); $gridBar.BorderThickness='1'; $gridBar.CornerRadius='10'
    $gridText=[Windows.Controls.TextBlock]::new(); $gridText.Text=('Grid: {0}%   Scanlines: {1}%   Glow: {2}%' -f [int]($script:CurrentGrid*100), [int]($script:CurrentScanlines*100), [int]($script:CurrentGlow*100)); $gridText.Foreground='White'; $gridBar.Child=$gridText; $pv.Children.Add($gridBar)|Out-Null

    $preview.Child=$pv; $rstack.Children.Add($preview)|Out-Null; $right.Child=$rstack
    $host.Children.Add($left)|Out-Null; $host.Children.Add($right)|Out-Null
    $CardPanel.Children.Add($host)|Out-Null
}
...
(& $find 'ApplyBtn').Add_Click({
    ...
    if($r -ne 'Yes') { return }
    try {
        Invoke-TweakCommands $selected
        Mark-TweaksApplied $selected
        foreach($item in $selected) { if($script:CheckboxMap.ContainsKey($item.Id)) { Set-TweakVisualState $item.Id } }
        Update-SelectedCount
        [System.Windows.MessageBox]::Show('Finished applying selected tweaks. Reboot your PC for full effect.') | Out-Null
    } catch {
