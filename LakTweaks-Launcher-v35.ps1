...
function Set-TweakFailedVisualState([string]$id, [string]$reason = '') {
    ...
    $entry.Card.Background     = Get-ArgbBrush '#ff4757' 0.06
}
...
function New-LakRestorePoint {
    try {
        Write-Log 'Creating restore point: LakTweaks v35 Before Apply...' 'INFO'
        Checkpoint-Computer -Description 'LakTweaks v35 Before Apply' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
...
function Get-LakHardwareSummary {
    ...
    try { $osObj = Get-CimInstance Win32_OperatingSystem; $os = ($osObj.Caption + ' ' + $osObj.BuildNumber); $ram = [math]::Round(($osObj.TotalVisibleMemorySize/1MB),1).ToString() + ' GB RAM' } catch {}
    return [pscustomobject]@{ CPU=$cpu; GPU=$gpu; OS=$os; RAM=$ram }
}

function Update-PerformancePanel {
    try {
        if ($null -ne $WindowsValueText) {
            $build = (Get-CimInstance Win32_OperatingSystem -ErrorAction SilentlyContinue).BuildNumber
            if ($build) { $WindowsValueText.Text = $build }
        }
...
function Invoke-TweakCommands([object[]]$selected) {
    ...
    foreach($item in $selected) {
        ...
        try {
            ...
            $output = ''
            if (Test-Path $tempLog) { $output = Get-Content -Path $tempLog -Raw -ErrorAction SilentlyContinue }
            if (-not [string]::IsNullOrWhiteSpace($output)) {
                $short = ($output -split "`r?`n" | Where-Object { $_.Trim() } | Select-Object -Last 5) -join ' | '
                Write-Log ($item.Title + ' output: ' + $short) 'OUT'
            }
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="LakTweaks v35 GOD"
        Height="940" Width="1680"
  ...
  <Grid>
    ...
    <!-- HEADER -->
    <Border x:Name="HeaderBorder" Grid.Row="0" Background="#060b12" BorderThickness="0,0,0,1">
      <Grid Margin="18,0">
        ...
        <!-- App logo -->
        <Border Width="62" Height="62" CornerRadius="16" Background="#07131c" BorderThickness="1" BorderBrush="#173c45" VerticalAlignment="Center" Margin="0,0,16,0">
          <TextBlock Text="⚡" FontSize="36" FontWeight="Black" HorizontalAlignment="Center" VerticalAlignment="Center" Foreground="#00ffe7"/>
        </Border>
        ...
        <StackPanel Grid.Column="1" Orientation="Vertical" VerticalAlignment="Center">
          <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
            ...
            <TextBlock Text=" v35" FontSize="26" FontWeight="Bold" Foreground="#00ffe7" Margin="10,0,0,0"/>
            <Border CornerRadius="6" Padding="10,2" Margin="12,2,0,0" Background="#20103a" BorderBrush="#784dff" BorderThickness="1">
              <TextBlock Text="GOD" FontSize="14" FontWeight="Bold" Foreground="#b98cff"/>
            </Border>
          </StackPanel>
          <TextBlock Text="ULTIMATE WINDOWS OPTIMIZER" FontSize="12" Foreground="#6fa19b" Margin="1,5,0,0"/>
        </StackPanel>

        <!-- Status cards / theme buttons -->
        <StackPanel Grid.Column="2" Orientation="Horizontal" VerticalAlignment="Center">
          <Border CornerRadius="12" Padding="14,8" Margin="0,0,10,0" Background="#08141d" BorderBrush="#12313a" BorderThickness="1">
            <StackPanel Orientation="Horizontal">
              <TextBlock Text="▣" Foreground="#00ffe7" FontSize="18" Margin="0,0,8,0"/>
              <StackPanel>
                <TextBlock Text="Windows" Foreground="#ffffff" FontSize="11" FontWeight="Bold"/>
                <TextBlock x:Name="WindowsValueText" Text="Detecting" Foreground="#00ffe7" FontSize="10"/>
              </StackPanel>
            </StackPanel>
          </Border>
          <Border CornerRadius="12" Padding="14,8" Margin="0,0,10,0" Background="#08141d" BorderBrush="#12313a" BorderThickness="1">
            <StackPanel Orientation="Horizontal">
              <TextBlock Text="▰" Foreground="#21d4fd" FontSize="18" Margin="0,0,8,0"/>
              <StackPanel>
                <TextBlock Text="CPU" Foreground="#ffffff" FontSize="11" FontWeight="Bold"/>
                <TextBlock x:Name="CpuValueText" Text="--%" Foreground="#00ffe7" FontSize="10"/>
              </StackPanel>
            </StackPanel>
          </Border>
          <Border CornerRadius="12" Padding="14,8" Margin="0,0,10,0" Background="#08141d" BorderBrush="#12313a" BorderThickness="1">
            <StackPanel Orientation="Horizontal">
              <TextBlock Text="▥" Foreground="#00ffe7" FontSize="18" Margin="0,0,8,0"/>
              <StackPanel>
                <TextBlock Text="RAM" Foreground="#ffffff" FontSize="11" FontWeight="Bold"/>
                <TextBlock x:Name="RamValueText" Text="--%" Foreground="#00ffe7" FontSize="10"/>
              </StackPanel>
            </StackPanel>
          </Border>
          <Border CornerRadius="12" Padding="14,8" Margin="0,0,16,0" Background="#08141d" BorderBrush="#12313a" BorderThickness="1">
            <StackPanel Orientation="Horizontal">
              <TextBlock Text="⛨" Foreground="#2ed573" FontSize="18" Margin="0,0,8,0"/>
              <StackPanel>
                <TextBlock Text="Admin" Foreground="#ffffff" FontSize="11" FontWeight="Bold"/>
                <TextBlock x:Name="AdminValueText" Text="Checking" Foreground="#2ed573" FontSize="10"/>
              </StackPanel>
            </StackPanel>
          </Border>
          <Ellipse Width="8" Height="8" Fill="#2ed573" Margin="0,0,6,0" VerticalAlignment="Center"/>
          <TextBlock Text="READY" Foreground="#2ed573" VerticalAlignment="Center" FontWeight="SemiBold" FontSize="11" Margin="0,0,16,0"/>
    ...
    <Border x:Name="FooterBorder" Grid.Row="2" BorderThickness="0,1,0,0">
      <Grid Margin="18,8">
        ...
        <WrapPanel Grid.Row="0" Grid.Column="0" VerticalAlignment="Center">
          <Button x:Name="SafePresetBtn"    Content="🛡  SAFE PRESET"       Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="CompPresetBtn"    Content="🎯  COMPETITIVE"       Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="ExtremePresetBtn" Content="☠  EXTREME MODE"      Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="SelectAllBtn"     Content="▣  SELECT ALL"        Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="ResetAllBtn"      Content="↻  RESET ALL"         Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="ExportBatBtn"     Content="▤  EXPORT BAT"        Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="RestorePointBtn"  Content="🛡  RESTORE POINT"     Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="RetryFailedBtn"   Content="↻  RETRY FAILED"      Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="ClearLogBtn"      Content="🧹  CLEAR LOG"         Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="OptimizeBtn"      Content="🚀  OPTIMIZE MY PC"    Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="LowTimerBtn"      Content="⏱  LOW TIMER"         Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="ApplyBtn"         Content="▶  APPLY SELECTED" Padding="18,9" Margin="0,0,8,8" FontWeight="Bold"/>
        </WrapPanel>
        <StackPanel Grid.Row="0" Grid.Column="1" VerticalAlignment="Top" Margin="12,0,4,0">
        ...
        <Border Grid.Row="1" Grid.ColumnSpan="2" Margin="0,0,0,0" CornerRadius="10" BorderThickness="1" BorderBrush="#15343d" Background="#050b10" Height="52" VerticalAlignment="Top">
          <Grid>
            <Grid.ColumnDefinitions><ColumnDefinition Width="Auto"/><ColumnDefinition Width="*"/></Grid.ColumnDefinitions>
            <TextBlock Text="✓" FontSize="20" FontWeight="Bold" Foreground="#00ffe7" Margin="12,0,8,0" VerticalAlignment="Center"/>
            <TextBox x:Name="LogBox"
                     ...
                     FontWeight="Bold"
                     Padding="0,13,8,0"
                     Text="LakTweaks v35 GOD log ready. Pick a preset or select tweaks manually."/>
          </Grid>
        </Border>
      </Grid>
    </Border>
...
(& $find 'ClearLogBtn').Add_Click({
    if ($null -ne $LogBox) { $LogBox.Text = 'LakTweaks v35 GOD log cleared.' }
...
(& $find 'ApplyBtn').Add_Click({
    ...
    $r = [System.Windows.MessageBox]::Show(
        ...
        'LakTweaks v35 GOD','YesNo','Warning')
    if ($r -ne 'Yes') { return }
    ...
    try {
        ...
        if ($result.Failed.Count -gt 0) {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count), failed $($result.Failed.Count). Check the log panel, then use RETRY FAILED if needed.", 'LakTweaks v35 GOD', 'OK', 'Warning') | Out-Null
        } else {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count) tweak(s) successfully.`nReboot your PC for full effect.", 'LakTweaks v35 GOD', 'OK', 'Information') | Out-Null
        }
    } catch {
...
Write-Log ('Hardware detected: ' + $hwStart.OS + ' | ' + $hwStart.CPU + ' | ' + $hwStart.GPU + ' | ' + $hwStart.RAM) 'INFO'
