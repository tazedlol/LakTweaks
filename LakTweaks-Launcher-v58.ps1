...
function New-LakRestorePoint {
    try {
        Write-Log 'Creating restore point: LakTweaks v58 Before Apply...' 'INFO'
        Checkpoint-Computer -Description 'LakTweaks v58 Before Apply' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
...
function Backup-LakFullRegistry {
    try {
        ...
        [System.Windows.MessageBox]::Show('Full registry backup saved to:`n' + $dir, 'LakTweaks v58', 'OK', 'Information') | Out-Null
...
function Restore-LakFullRegistry {
    try {
        ...
        $r = [System.Windows.MessageBox]::Show('Restore the saved full registry backup now? You should reboot after restoring.', 'LakTweaks v58', 'YesNo', 'Warning')
        if ($r -ne 'Yes') { return }
  ...
  <Grid>
    ...
    <Border x:Name="FooterBorder" Grid.Row="2" Grid.Column="1" Margin="16,0,16,12" CornerRadius="14" Background="#071018" BorderBrush="#00a7c8" BorderThickness="1.1">
      <Grid Margin="22,0">
        ...
        <Border Grid.Column="3" CornerRadius="10" BorderBrush="#00a7c8" BorderThickness="1" Padding="16,7" HorizontalAlignment="Center" VerticalAlignment="Center"><TextBlock Text="v57.0.0" Foreground="#00ffe7" FontSize="13" FontWeight="Black"/></Border>
      </Grid>
    </Border>
...
function Invoke-ApplySelected {
    ...
    try {
        ...
        if ($result.Failed.Count -gt 0) {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count), failed $($result.Failed.Count). Check the log, then use RETRY FAILED.", 'LakTweaks v58', 'OK', 'Warning') | Out-Null
        } else {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count) tweak(s) successfully.`nReboot your PC for full effect.", 'LakTweaks v58', 'OK', 'Information') | Out-Null
        }
    } catch {
...
function Show-Section([string]$key) {
    ...
    # Per-section Apply Selected footer button
    $applyBar = [Windows.Controls.Border]::new()
    $applyBar.Margin          = '0,18,0,4'
    $applyBar.Padding         = '16,10'
    $applyBar.CornerRadius    = '12'
    $applyBar.Background      = Get-ArgbBrush '#071f14' 0.95
    $applyBar.BorderBrush     = Get-ArgbBrush '#2ed573' 0.45
    $applyBar.BorderThickness = '1'

    $applyRow = [Windows.Controls.StackPanel]::new()
    $applyRow.Orientation = 'Horizontal'
    $applyRow.HorizontalAlignment = 'Center'

    $applySecBtn = [Windows.Controls.Button]::new()
    $applySecBtn.Content   = '▶  APPLY SELECTED TWEAKS'
    $applySecBtn.FontSize  = 13
    $applySecBtn.FontWeight = 'Black'
    $applySecBtn.Foreground = Get-ColorBrush '#2ed573'
    $applySecBtn.Background = Get-ArgbBrush '#2ed573' 0.10
    $applySecBtn.BorderBrush = Get-ArgbBrush '#2ed573' 0.55
    $applySecBtn.BorderThickness = '1'
    $applySecBtn.Padding   = '32,12'
    $applySecBtn.Margin    = '0,0,20,0'
    $applySecBtn.Add_Click({ Invoke-ApplySelected })
    $applyRow.Children.Add($applySecBtn) | Out-Null

    $selInfo = [Windows.Controls.TextBlock]::new()
    $selInfo.VerticalAlignment = 'Center'
    $selInfo.Foreground = Get-ColorBrush '#7ca7a2'
    $selInfo.FontSize = 12
    $selInfo.Text = 'Selections persist across all sections. Check tweaks in any section, then apply.'
    $applyRow.Children.Add($selInfo) | Out-Null

    $applyBar.Child = $applyRow
    # Add as a full-width WrapPanel item by wrapping in a container
    $fullW = [Windows.Controls.Border]::new()
    $fullW.Width = [double]::NaN
    $fullW.HorizontalAlignment = 'Stretch'
    $fullW.Child = $applyBar
    $CardPanel.Children.Add($fullW) | Out-Null
...
(& $find 'ClearLogBtn').Add_Click({
    if ($null -ne $LogBox) { $LogBox.Text = 'LakTweaks v58 log cleared.' }
...
Write-Log 'v56 PRO: live time fixed, CPU/RAM use lightweight counters, hardware-aware engine active.' 'OK'
Show-Section 'home'
Update-SelectedCount
