...
function New-LakRestorePoint {
    try {
        Write-Log 'Creating restore point: LakTweaks v57 Before Apply...' 'INFO'
        Checkpoint-Computer -Description 'LakTweaks v57 Before Apply' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
...
(& $find 'ClearLogBtn').Add_Click({
    if ($null -ne $LogBox) { $LogBox.Text = 'LakTweaks v57 log cleared.' }
