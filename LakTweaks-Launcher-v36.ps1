...
function Update-PerformancePanel {
    try {
        ...
        if ($null -ne $CpuValueText -and $script:CpuCounter) { $CpuValueText.Text = ([math]::Round($script:CpuCounter.NextValue(),0).ToString() + '%') }
        if ($null -ne $RamValueText -and $script:RamCounter) { $RamValueText.Text = ([math]::Round($script:RamCounter.NextValue(),0).ToString() + '%') }
    } catch {}
}
      ...
      <Grid Margin="18,0">
        ...
        <StackPanel Grid.Column="1" Orientation="Vertical" VerticalAlignment="Center">
          <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
            ...
            <TextBlock Text=" v36" FontSize="26" FontWeight="Bold" Foreground="#00ffe7" Margin="10,0,0,0"/>
            <Border CornerRadius="6" Padding="10,2" Margin="12,2,0,0" Background="#20103a" BorderBrush="#784dff" BorderThickness="1">
              <TextBlock Text="ULTIMATE" FontSize="14" FontWeight="Bold" Foreground="#b98cff"/>
            </Border>
          </StackPanel>
    ...
    <Border x:Name="FooterBorder" Grid.Row="2" BorderThickness="0,1,0,0">
      <Grid Margin="18,8">
        ...
        <WrapPanel Grid.Row="0" Grid.Column="0" VerticalAlignment="Center">
          ...
          <Button x:Name="SmartNetBtn"      Content="🌐  SMART NETWORK"     Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="FullBackupBtn"    Content="💾  FULL BACKUP"       Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="RestoreBackupBtn" Content="↩  RESTORE BACKUP"     Padding="14,9" Margin="0,0,8,8"/>
          <Button x:Name="RamCleanBtn"      Content="🧠  RAM CLEAN"         Padding="14,9" Margin="0,0,8,8"/>
        ...
        <Border Grid.Row="1" Grid.ColumnSpan="2" Margin="0,0,0,0" CornerRadius="10" BorderThickness="1" BorderBrush="#15343d" Background="#050b10" Height="52" VerticalAlignment="Top">
          <Grid>
            ...
            <TextBox x:Name="LogBox"
                     ...
                     FontWeight="Bold"
                     Padding="0,13,8,0"
                     Text="LakTweaks v36 ULTIMATE log ready. Pick a preset or select tweaks manually."/>
          </Grid>
        </Border>
      </Grid>
    </Border>
...
(& $find 'ClearLogBtn').Add_Click({
    if ($null -ne $LogBox) { $LogBox.Text = 'LakTweaks v36 ULTIMATE log cleared.' }
...
Write-Log 'v36 speed mode enabled: cached stats, lighter logging, faster UI ticks.' 'OK'
$script:PerfTimer = [Windows.Threading.DispatcherTimer]::new()
$script:PerfTimer.Interval = [TimeSpan]::FromSeconds(6)
