...
function New-LakRestorePoint {
    try {
        Write-Log 'Creating restore point: LakTweaks v53 Before Apply...' 'INFO'
        Checkpoint-Computer -Description 'LakTweaks v53 Before Apply' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
...
function Initialize-FastPerformanceCounters {
    # Prime the CPU counter - first NextValue() always returns 0, second gives real delta
    $script:CpuCounterPrimed = $false
    try {
        $script:CpuCounter = [System.Diagnostics.PerformanceCounter]::new('Processor', '% Processor Time', '_Total')
        [void]$script:CpuCounter.NextValue()   # discard - always 0
    } catch { $script:CpuCounter = $null }
    # RAM: skip PerformanceCounter (measures virtual/paged memory = wrong)
    # Use WMI FreePhysicalMemory instead for accurate physical RAM %
    $script:RamCounter = $null
    $script:RamTickCount = 0
    $script:CpuCounterPrimed = $false
    try {
    ...
    } catch { $script:LastRamPct = '0%' }
}

function Update-PerformancePanel {
    try {
        if ($null -ne $WindowsValueText -and $script:CachedWindowsBuild) {
            $WindowsValueText.Text = [string]$script:CachedWindowsBuild
        }
        if ($null -ne $AdminValueText) {
            $AdminValueText.Text = if (Test-Admin) { 'Elevated' } else { 'Limited' }
        }
        if ($null -ne $LastUpdatedText) {
            $LastUpdatedText.Text = (Get-Date -Format 'M/d/yyyy  hh:mm:ss tt')
        }
        # CPU - PerformanceCounter gives real value from second call onwards (needs 1s delta)
        if ($null -ne $CpuValueText) {
            $cpuText = $null
            if ($script:CpuCounter) {
                try {
                    $raw = $script:CpuCounter.NextValue()
                    if (-not $script:CpuCounterPrimed) {
                        $script:CpuCounterPrimed = $true
                        # This is the first real 1s-delta reading after startup prime
                    }
                    $cpuText = [math]::Round($raw, 0).ToString() + '%'
                } catch {}
            }
            if (-not $cpuText) {
                try {
                    $load = (Get-CimInstance Win32_Processor -ErrorAction Stop | Measure-Object -Property LoadPercentage -Average).Average
                    if ($null -ne $load) { $cpuText = [math]::Round([double]$load, 0).ToString() + '%' }
                } catch {}
            }
            if ($cpuText) { $CpuValueText.Text = $cpuText }
        }
        # RAM - physical memory usage via WMI (cached every 4 ticks = ~8s to reduce CIM overhead)
        if ($null -ne $RamValueText) {
            $script:RamTickCount = ($script:RamTickCount + 1) % 4
            if ($script:RamTickCount -eq 0 -or -not $script:LastRamPct) {
                try {
                    $osObj = Get-CimInstance Win32_OperatingSystem -ErrorAction Stop
                    $used  = [double]($osObj.TotalVisibleMemorySize - $osObj.FreePhysicalMemory)
                    $pct   = [math]::Round(($used / [double]$osObj.TotalVisibleMemorySize) * 100, 0)
            ...
            if ($script:LastRamPct) { $RamValueText.Text = $script:LastRamPct }
        }
    } catch {}
}
...
function Backup-LakFullRegistry {
    try {
        ...
        [System.Windows.MessageBox]::Show('Full registry backup saved to:`n' + $dir, 'LakTweaks v53 AUTO OPTIMIZER', 'OK', 'Information') | Out-Null
...
function Restore-LakFullRegistry {
    try {
        ...
        $r = [System.Windows.MessageBox]::Show('Restore the saved full registry backup now? You should reboot after restoring.', 'LakTweaks v53 AUTO OPTIMIZER', 'YesNo', 'Warning')
        if ($r -ne 'Yes') { return }
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="LakTweaks v53 AUTO OPTIMIZER"
  ...
  <Grid>
      ...
      <Grid Margin="22,0,22,0">
        ...
        <StackPanel Orientation="Vertical" VerticalAlignment="Center">
          <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
            ...
            <TextBlock Text=" v53" FontSize="25" FontWeight="Black" Foreground="#00ffe7" Margin="14,0,0,0"/>
    ...
    <Border x:Name="FooterBorder" Grid.Row="2" Grid.Column="1" Margin="16,0,16,12" CornerRadius="14" Background="#071018" BorderBrush="#00a7c8" BorderThickness="1.1">
      <Grid Margin="22,0">
        ...
        <Border Grid.Column="3" CornerRadius="10" BorderBrush="#00a7c8" BorderThickness="1" Padding="16,7" HorizontalAlignment="Center" VerticalAlignment="Center"><TextBlock Text="v53.0.0" Foreground="#00ffe7" FontSize="13" FontWeight="Black"/></Border>
      </Grid>
    </Border>
...
function Set-HomeDashboardVisible([bool]$visible) {
    if ($visible) {
        ...
        $HomeRowDef.Height = [Windows.GridLength]::new(270)
    } else {
...
(& $find 'ClearLogBtn').Add_Click({
    if ($null -ne $LogBox) { $LogBox.Text = 'LakTweaks v53 log cleared.' }
...
(& $find 'ApplyBtn').Add_Click({
    ...
    $r = [System.Windows.MessageBox]::Show(
        ...
        'LakTweaks v53 AUTO OPTIMIZER','YesNo','Warning')
    if ($r -ne 'Yes') { return }
    ...
    try {
        ...
        if ($result.Failed.Count -gt 0) {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count), failed $($result.Failed.Count). Check the log panel, then use RETRY FAILED if needed.", 'LakTweaks v53 AUTO OPTIMIZER', 'OK', 'Warning') | Out-Null
        } else {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count) tweak(s) successfully.`nReboot your PC for full effect.", 'LakTweaks v53 AUTO OPTIMIZER', 'OK', 'Information') | Out-Null
        }
    } catch {
...
Write-Log 'v53: CPU counter primed correctly, RAM uses physical WMI metric, v47 Home layout.' 'OK'
