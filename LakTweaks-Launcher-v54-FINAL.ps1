...
function New-LakRestorePoint {
    try {
        Write-Log 'Creating restore point: LakTweaks v54 Before Apply...' 'INFO'
        Checkpoint-Computer -Description 'LakTweaks v54 Before Apply' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
...
function Initialize-FastPerformanceCounters {
    # v54 FINAL: no UI-thread PerformanceCounter loops.
    # CPU/RAM sampled by lightweight CIM calls on a slow timer.
    $script:LastCpuPct = '...'
    $script:LastRamPct = '...'
    $script:PerfTickCount = 0
    try {
...
function Update-PerformancePanel {
    try {
        ...
        # Poll CIM every other UI tick for smoothness. Display cached values every tick.
        $script:PerfTickCount = ($script:PerfTickCount + 1) % 2
        if ($script:PerfTickCount -eq 0) {
            try {
                $cpuLoad = (Get-CimInstance Win32_Processor -ErrorAction Stop | Measure-Object -Property LoadPercentage -Average).Average
                if ($null -ne $cpuLoad) { $script:LastCpuPct = ([math]::Round([double]$cpuLoad,0).ToString() + '%') }
            } catch {}
...
function Backup-LakFullRegistry {
    try {
        ...
        [System.Windows.MessageBox]::Show('Full registry backup saved to:`n' + $dir, 'LakTweaks v54 FINAL', 'OK', 'Information') | Out-Null
...
function Restore-LakFullRegistry {
    try {
        ...
        $r = [System.Windows.MessageBox]::Show('Restore the saved full registry backup now? You should reboot after restoring.', 'LakTweaks v54 FINAL', 'YesNo', 'Warning')
        if ($r -ne 'Yes') { return }
  ...
  <Grid>
      ...
      <Grid Margin="22,0,22,0">
        ...
        <StackPanel Orientation="Vertical" VerticalAlignment="Center">
          <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
            ...
            <TextBlock Text=" v54" FontSize="25" FontWeight="Black" Foreground="#00ffe7" Margin="14,0,0,0"/>
    ...
    <Border x:Name="FooterBorder" Grid.Row="2" Grid.Column="1" Margin="16,0,16,12" CornerRadius="14" Background="#071018" BorderBrush="#00a7c8" BorderThickness="1.1">
      <Grid Margin="22,0">
        ...
        <Border Grid.Column="3" CornerRadius="10" BorderBrush="#00a7c8" BorderThickness="1" Padding="16,7" HorizontalAlignment="Center" VerticalAlignment="Center"><TextBlock Text="v54.0.0" Foreground="#00ffe7" FontSize="13" FontWeight="Black"/></Border>
      </Grid>
    </Border>
...
(& $find 'ClearLogBtn').Add_Click({
    if ($null -ne $LogBox) { $LogBox.Text = 'LakTweaks v54 log cleared.' }
...
(& $find 'ApplyBtn').Add_Click({
    ...
    $r = [System.Windows.MessageBox]::Show(
        ...
        'LakTweaks v54 FINAL','YesNo','Warning')
    if ($r -ne 'Yes') { return }
    ...
    try {
        ...
        if ($result.Failed.Count -gt 0) {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count), failed $($result.Failed.Count). Check the log panel, then use RETRY FAILED if needed.", 'LakTweaks v54 FINAL', 'OK', 'Warning') | Out-Null
        } else {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count) tweak(s) successfully.`nReboot your PC for full effect.", 'LakTweaks v54 FINAL', 'OK', 'Information') | Out-Null
        }
    } catch {
...
Write-Log 'v54 FINAL: perfect home layout, accurate WMI CPU/RAM stats, lag-reduced UI timer.' 'OK'
$script:PerfTimer = [Windows.Threading.DispatcherTimer]::new()
$script:PerfTimer.Interval = [TimeSpan]::FromSeconds(3)
