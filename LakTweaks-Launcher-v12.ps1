...
function Show-ErrorDialog([string]$message, [string]$title = 'LakTweaks Error') {
    try { [System.Windows.MessageBox]::Show($message, $title, 'OK', 'Error') | Out-Null } catch { Write-Error $message }
}

function Invoke-Safely([scriptblock]$Script, [string]$Context = 'LakTweaks') {
    try { & $Script }
    catch {
        $msg = $_.Exception.Message
        Show-ErrorDialog (($Context + ': ' + $msg)) $Context
    }
}
function Test-Admin {
...
try {
    ...
    Show-ErrorDialog ('The LakTweaks window failed to load. ' + $_.Exception.Message) 'LakTweaks Startup Error'
    return
}
...
$SidebarList.Add_SelectionChanged({ Invoke-Safely { if ($SidebarList.SelectedItem) { Show-Section ([string]$SidebarList.SelectedItem.Tag) } } 'Section Change' })
...
foreach($btnName in 'ThemeCyan','ThemePink','ThemeGold','ThemePurple','ThemeRed') {
    (& $find $btnName).Add_Click({
        ...
        Invoke-Safely {
            ...
            Save-UiSettings
            Apply-Theme
            if($SidebarList.SelectedItem -and $SidebarList.SelectedItem.Tag -eq 'ui'){Show-UiSettings}else{Show-Section ([string]$SidebarList.SelectedItem.Tag)}
        } 'Theme Change'
    })
}
...
try { Show-Section 'fortnite' } catch { Show-ErrorDialog ('The default section failed to load. ' + $_.Exception.Message) 'LakTweaks Startup Error' }
try { Update-SelectedCount } catch { }
try { $Window.ShowDialog() | Out-Null } catch { Show-ErrorDialog ('LakTweaks crashed while opening. ' + $_.Exception.Message) 'LakTweaks Runtime Error' }