  ...
  <Grid>
    ...
    <Border x:Name="HeaderBorder" Grid.Row="0" Background="#080f16" BorderThickness="0,0,0,1">
      <Grid Margin="22,0">
        ...
        <StackPanel Orientation="Vertical" VerticalAlignment="Center">
          <TextBlock x:Name="LogoText" Text="LAK TWEAKS ELITE" FontSize="22" FontWeight="Black" LetterSpacing="2"/>
    ...
    <Grid Grid.Row="1">
      ...
      <Border x:Name="SidebarBorder" Grid.Column="0" BorderThickness="0,0,1,0">
        <DockPanel Margin="10">
          <TextBlock DockPanel.Dock="Top" Text="SECTIONS" Margin="10,10,10,8" FontSize="10" FontWeight="Bold" Foreground="#5a8a85" LetterSpacing="1"/>
    ...
    <Border x:Name="FooterBorder" Grid.Row="2" BorderThickness="0,1,0,0">
      <Grid Margin="20,0">
        ...
        <StackPanel Grid.Column="1" VerticalAlignment="Center" Margin="0,0,4,0">
          <TextBlock Text="SELECTED" FontSize="10" Foreground="#5a8a85" HorizontalAlignment="Right" LetterSpacing="1"/>
...
function New-Card([object]$item) {
    $cardW = if ($script:CurrentCompact) { 322 } else { 340 }

    $outer = [Windows.Controls.Border]::new()
    $outer.Width          = $cardW
    $outer.MinHeight      = if ($script:CurrentCompact) { 196 } else { 222 }
    $outer.Margin         = '0,0,18,18'
    $outer.Padding        = if ($script:CurrentCompact) { '14,12' } else { '18,16' }
    $outer.CornerRadius   = '12'
    $outer.Background     = if ($script:CurrentClean) { Get-ArgbBrush '#0e171d' 0.92 } else { Get-ArgbBrush $script:CurrentAccent (0.03 + ($script:CurrentBgOpacity * 0.07)) }
    ...
    for ($r = 0; $r -lt 5; $r++) { [void]$grid.RowDefinitions.Add([Windows.Controls.RowDefinition]::new()) }
    ...
    $title.FontSize    = if ($script:CurrentCompact) { 13 } else { 14 }
    $title.FontWeight  = 'Bold'
    $title.Foreground  = [Windows.Media.Brushes]::White
    $title.TextWrapping = 'Wrap'
    $title.MaxWidth    = $cardW - 36
    ...
    $desc.MaxHeight    = if ($script:CurrentCompact) { 46 } else { 56 }
    ...
    # Status label — right-aligned, fixed width so it never clips
...
function Show-Section([string]$key) {
    ...
    $CardPanel.ItemWidth = if ($script:CurrentCompact) { 322 } else { 340 }
