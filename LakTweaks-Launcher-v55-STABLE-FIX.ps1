...
function New-LakRestorePoint {
    try {
        Write-Log 'Creating restore point: LakTweaks v55 Before Apply...' 'INFO'
        Checkpoint-Computer -Description 'LakTweaks v55 Before Apply' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
...
function Initialize-FastPerformanceCounters {
    # v55 STABLE: zero-lag stats. No live CIM/WMI loop on the UI thread.
    # Reads once on startup, then the UI simply displays cached values.
    $script:LastCpuPct = 'Ready'
    $script:LastRamPct = 'Ready'
    try {
        $cpuLoad = (Get-CimInstance Win32_Processor -ErrorAction Stop | Select-Object -First 1 -ExpandProperty LoadPercentage)
        if ($null -ne $cpuLoad) { $script:LastCpuPct = ([math]::Round([double]$cpuLoad,0).ToString() + '%') }
    } catch { $script:LastCpuPct = 'N/A' }
    try {
        $osObj = Get-CimInstance Win32_OperatingSystem -ErrorAction Stop
        $used  = [double]($osObj.TotalVisibleMemorySize - $osObj.FreePhysicalMemory)
        $pct   = [math]::Round(($used / [double]$osObj.TotalVisibleMemorySize) * 100, 0)
        $script:LastRamPct = $pct.ToString() + '%'
    } catch { $script:LastRamPct = 'N/A' }
}

function Update-PerformancePanel {
    # Lightweight display-only updater. No CIM/WMI here, so the UI will not lag or crash.
    try {
...
function Backup-LakFullRegistry {
    try {
        ...
        [System.Windows.MessageBox]::Show('Full registry backup saved to:`n' + $dir, 'LakTweaks v55 STABLE', 'OK', 'Information') | Out-Null
...
function Restore-LakFullRegistry {
    try {
        ...
        $r = [System.Windows.MessageBox]::Show('Restore the saved full registry backup now? You should reboot after restoring.', 'LakTweaks v55 STABLE', 'YesNo', 'Warning')
        if ($r -ne 'Yes') { return }
  ...
  <Grid>
      ...
      <Grid Margin="22,0,22,0">
        ...
        <StackPanel Orientation="Vertical" VerticalAlignment="Center">
          <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
            ...
            <TextBlock Text=" v55" FontSize="25" FontWeight="Black" Foreground="#00ffe7" Margin="14,0,0,0"/>
    ...
    <Border x:Name="FooterBorder" Grid.Row="2" Grid.Column="1" Margin="16,0,16,12" CornerRadius="14" Background="#071018" BorderBrush="#00a7c8" BorderThickness="1.1">
      <Grid Margin="22,0">
        ...
        <Border Grid.Column="3" CornerRadius="10" BorderBrush="#00a7c8" BorderThickness="1" Padding="16,7" HorizontalAlignment="Center" VerticalAlignment="Center"><TextBlock Text="v55.0.0" Foreground="#00ffe7" FontSize="13" FontWeight="Black"/></Border>
      </Grid>
    </Border>
...
(& $find 'ClearLogBtn').Add_Click({
    if ($null -ne $LogBox) { $LogBox.Text = 'LakTweaks v55 log cleared.' }
...
(& $find 'ApplyBtn').Add_Click({
    ...
    $r = [System.Windows.MessageBox]::Show(
        ...
        'LakTweaks v55 STABLE','YesNo','Warning')
    if ($r -ne 'Yes') { return }
    ...
    try {
        ...
        if ($result.Failed.Count -gt 0) {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count), failed $($result.Failed.Count). Check the log panel, then use RETRY FAILED if needed.", 'LakTweaks v55 STABLE', 'OK', 'Warning') | Out-Null
        } else {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count) tweak(s) successfully.`nReboot your PC for full effect.", 'LakTweaks v55 STABLE', 'OK', 'Information') | Out-Null
        }
    } catch {
...
Write-Log 'v55 STABLE: perfect home layout, accurate WMI CPU/RAM stats, lag-reduced UI timer.' 'OK'
Show-Section 'home'
Update-SelectedCount
