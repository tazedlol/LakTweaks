...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
  ...
  <Grid Margin="16">
    ...
    <TextBlock x:Name="PromptText" Text="Enter any hex accent color" FontSize="16" FontWeight="SemiBold"/>
    <TextBlock Grid.Row="1" Margin="0,8,0,12" Text="#RGB or #RRGGBB examples: #F00, #00FFE7, #7C3AED" Foreground="#7ca7a2"/>
...
function New-Card([object]$item) {
    ...
    $outer.Width = if($script:CurrentCompact) {320} else {346}
    $outer.MinHeight = if($script:CurrentCompact) {198} else {218}
    $outer.Margin = '0,0,20,20'
    $outer.Padding = if($script:CurrentCompact){'14'}else{'20'}
    ...
    $title.MaxWidth=if($script:CurrentCompact){272}else{292}
    ...
    $desc.MaxHeight=if($script:CurrentCompact){50}else{58}
...
function Show-UiSettings {
    ...
    $accentPreview.ToolTip='Click to enter any hex color'
...
function Show-Section([string]$key) {
    ...
    $CardPanel.ItemWidth = if($script:CurrentCompact){320}else{346}
