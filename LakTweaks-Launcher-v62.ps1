...
[
  ...
  {
    "CategoryId": "smart",
    "Category": "SMART ENGINE",
    "Id": "smart_atom_core_safe",
    "Title": "Atom OS Safe Core",
    "Description": "Replicates the useful Atom OS-style baseline without nuking core Windows services: telemetry off, background apps off, GameDVR off, FSE-safe fullscreen keys.",
    "Command": ":: === Atom OS Safe Core ===\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\DataCollection\" /v \"AllowTelemetry\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\DataCollection\" /v \"AllowTelemetry\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\BackgroundAccessApplications\" /v \"GlobalUserDisabled\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Search\" /v \"BackgroundAppGlobalToggle\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v \"GameDVR_Enabled\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v \"GameDVR_FSEBehaviorMode\" /t REG_DWORD /d 2 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v \"GameDVR_DXGIHonorFSEWindowsCompatible\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v \"GameDVR_HonorUserFSEBehaviorMode\" /t REG_DWORD /d 1 /f >nul 2>&1\necho [OK] Atom OS safe core applied"
  },
  {
    "CategoryId": "smart",
    "Category": "SMART ENGINE",
    "Id": "smart_winaero_responsive_shell",
    "Title": "Winaero Responsiveness Pack",
    "Description": "Safe Winaero-style responsiveness tweaks: faster menus, no startup delay, fewer animations, no transparency, and snappier Explorer behavior.",
    "Command": ":: === Winaero Responsiveness Pack ===\nreg add \"HKCU\\Control Panel\\Desktop\" /v \"MenuShowDelay\" /t REG_SZ /d \"0\" /f >nul 2>&1\nreg add \"HKCU\\Control Panel\\Desktop\" /v \"AutoEndTasks\" /t REG_SZ /d \"1\" /f >nul 2>&1\nreg add \"HKCU\\Control Panel\\Desktop\" /v \"HungAppTimeout\" /t REG_SZ /d \"1000\" /f >nul 2>&1\nreg add \"HKCU\\Control Panel\\Desktop\" /v \"WaitToKillAppTimeout\" /t REG_SZ /d \"2000\" /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\" /v \"WaitToKillServiceTimeout\" /t REG_SZ /d \"2000\" /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Serialize\" /v \"StartupDelayInMSec\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize\" /v \"EnableTransparency\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Control Panel\\Desktop\\WindowMetrics\" /v \"MinAnimate\" /t REG_SZ /d \"0\" /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Advanced\" /v \"TaskbarAnimations\" /t REG_DWORD /d 0 /f >nul 2>&1\necho [OK] Winaero responsiveness pack applied"
  },
  {
    "CategoryId": "smart",
    "Category": "SMART ENGINE",
    "Id": "smart_wpd_privacy_core",
    "Title": "WPD Privacy Core",
    "Description": "WPD-style privacy controls without breaking Windows Update or networking: telemetry, feedback, activity history, advertising ID, and content suggestions off.",
    "Command": ":: === WPD Privacy Core ===\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\DataCollection\" /v \"AllowTelemetry\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\DataCollection\" /v \"DoNotShowFeedbackNotifications\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\System\" /v \"EnableActivityFeed\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\System\" /v \"PublishUserActivities\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\System\" /v \"UploadUserActivities\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\SOFTWARE\\Microsoft\\Siuf\\Rules\" /v \"NumberOfSIUFInPeriod\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg delete \"HKCU\\SOFTWARE\\Microsoft\\Siuf\\Rules\" /v \"PeriodInNanoSeconds\" /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\AdvertisingInfo\" /v \"DisabledByGroupPolicy\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager\" /v \"ContentDeliveryAllowed\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager\" /v \"SystemPaneSuggestionsEnabled\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\CloudContent\" /v \"DisableWindowsConsumerFeatures\" /t REG_DWORD /d 1 /f >nul 2>&1\necho [OK] WPD privacy core applied"
  },
  {
    "CategoryId": "smart",
    "Category": "SMART ENGINE",
    "Id": "smart_safe_service_trim",
    "Title": "Safe Service Trim",
    "Description": "Atom/WPD-style safe service trim. Disables telemetry, maps, Xbox, reporting, retail demo, and push junk while avoiding EventLog, SamSs, PlugPlay, HID, and core networking.",
    "Command": ":: === Safe Service Trim ===\nfor %%S in (DiagTrack dmwappushservice diagnosticshub.standardcollector.service WerSvc Wecsvc MapsBroker RetailDemo XblAuthManager XblGameSave XboxGipSvc XboxNetApiSvc BcastDVRUserService CaptureService WpnService WpnUserService PushToInstall WalletService Fax) do (\n    sc stop %%S >nul 2>&1\n    sc config %%S start= disabled >nul 2>&1\n)\necho [OK] Safe service trim applied"
  },
  {
    "CategoryId": "smart",
    "Category": "SMART ENGINE",
    "Id": "smart_competitive_scheduler",
    "Title": "Competitive Scheduler Core",
    "Description": "Conflict-free scheduler/game core: Win32PrioritySeparation 26, MMCSS games priority, no network throttling, and safe fullscreen keys.",
    "Command": ":: === Competitive Scheduler Core ===\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\PriorityControl\" /v \"Win32PrioritySeparation\" /t REG_DWORD /d 26 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\" /v \"SystemResponsiveness\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\" /v \"NetworkThrottlingIndex\" /t REG_DWORD /d 4294967295 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games\" /v \"GPU Priority\" /t REG_DWORD /d 8 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games\" /v \"Priority\" /t REG_DWORD /d 6 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games\" /v \"Scheduling Category\" /t REG_SZ /d \"High\" /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games\" /v \"SFIO Priority\" /t REG_SZ /d \"High\" /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v \"GameDVR_FSEBehaviorMode\" /t REG_DWORD /d 2 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v \"GameDVR_DXGIHonorFSEWindowsCompatible\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v \"GameDVR_HonorUserFSEBehaviorMode\" /t REG_DWORD /d 1 /f >nul 2>&1\necho [OK] Competitive scheduler core applied"
  },
  {
    "CategoryId": "smart",
    "Category": "SMART ENGINE",
    "Id": "smart_network_stability",
    "Title": "Smart Network Stability",
    "Description": "Gaming-safe network baseline: per-adapter TCPNoDelay/ACK settings, QoS 0%, DNS priority, RSS enabled, RSC off, and NIC power saving disabled.",
    "Command": ":: === Smart Network Stability ===\nnetsh int tcp set global ecncapability=disabled >nul 2>&1\nnetsh int tcp set global timestamps=disabled >nul 2>&1\nnetsh int tcp set heuristics disabled >nul 2>&1\nnetsh int tcp set global rss=enabled >nul 2>&1\nnetsh int tcp set global rsc=disabled >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\Psched\" /v \"NonBestEffortLimit\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\ServiceProvider\" /v \"LocalPriority\" /t REG_DWORD /d 4 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\ServiceProvider\" /v \"HostsPriority\" /t REG_DWORD /d 5 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\ServiceProvider\" /v \"DnsPriority\" /t REG_DWORD /d 6 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\ServiceProvider\" /v \"NetbtPriority\" /t REG_DWORD /d 7 /f >nul 2>&1\nfor /f \"tokens=*\" %%i in ('reg query \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters\\Interfaces\" ^| findstr /i \"HKEY_LOCAL_MACHINE\"') do (\n    reg add \"%%i\" /v \"TCPNoDelay\" /t REG_DWORD /d 1 /f >nul 2>&1\n    reg add \"%%i\" /v \"TcpAckFrequency\" /t REG_DWORD /d 1 /f >nul 2>&1\n    reg add \"%%i\" /v \"TcpDelAckTicks\" /t REG_DWORD /d 0 /f >nul 2>&1\n)\npowershell -NoProfile -ExecutionPolicy Bypass -Command \"Get-NetAdapter -Physical -ErrorAction SilentlyContinue | ForEach-Object { Set-NetAdapterPowerManagement -Name $_.Name -AllowComputerToTurnOffDevice Disabled -ErrorAction SilentlyContinue }\" >nul 2>&1\necho [OK] Smart network stability applied"
  },
  {
    "CategoryId": "smart",
    "Category": "SMART ENGINE",
    "Id": "smart_startup_silent_cleanup",
    "Title": "Silent Startup Cleanup",
    "Description": "Safe startup cleanup inspired by custom OS debloat: removes common launchers from startup without deleting programs.",
    "Command": ":: === Silent Startup Cleanup ===\nreg delete \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\" /v \"OneDrive\" /f >nul 2>&1\nreg delete \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\" /v \"Discord\" /f >nul 2>&1\nreg delete \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\" /v \"Spotify\" /f >nul 2>&1\nreg delete \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\" /v \"Steam\" /f >nul 2>&1\nreg delete \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\" /v \"EpicGamesLauncher\" /f >nul 2>&1\nreg delete \"HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\" /v \"OneDrive\" /f >nul 2>&1\nreg delete \"HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\" /v \"SecurityHealth\" /f >nul 2>&1\necho [OK] Silent startup cleanup applied"
  },
  {
    "CategoryId": "input",
    "Category": "INPUT / MOUSE",
    "Id": "device_powersave_wmi_off",
    "Title": "Disable Device Power Saving (WMI)",
    "Description": "Uses MSPower_DeviceEnable to disable device-level power saving. Based on your Disable Device Powersaving.ps1.",
    "Command": ":: === Disable Device Power Saving WMI ===\npowershell -NoProfile -ExecutionPolicy Bypass -Command \"try { Get-WmiObject MSPower_DeviceEnable -Namespace root\\wmi | ForEach-Object { $_.enable = $false; $_.psbase.put() | Out-Null }; Write-Host '[OK] Device power saving disabled through WMI' } catch { Write-Host '[WARN] WMI device power saving skipped:' $_.Exception.Message }\"\necho [OK] Device power saving WMI pass finished"
  },
  {
    "CategoryId": "input",
    "Category": "INPUT / MOUSE",
    "Id": "usb_idle_power_saving_off",
    "Title": "Disable USB Idle + Selective Suspend",
    "Description": "Disables common USB/controller idle and selective suspend registry values from your USB idle scripts.",
    "Command": ":: === Disable USB Idle / Selective Suspend ===\nfor %%A in (EnhancedPowerManagementEnabled AllowIdleIrpInD3 SelectiveSuspendEnabled DeviceSelectiveSuspended D3ColdSupported DeviceIdleEnabled EnableIdlePowerManagement) do (\n  for /f \"tokens=*\" %%B in ('reg query \"HKLM\\SYSTEM\\CurrentControlSet\\Enum\\USB\" /s /v \"%%A\" 2^>nul ^| findstr /i \"HKEY_LOCAL_MACHINE\"') do (\n    reg add \"%%B\" /v \"%%A\" /t REG_DWORD /d 0 /f >nul 2>&1\n  )\n)\npowercfg /setacvalueindex scheme_current 2a737441-1930-4402-8d77-b2bebba308a3 d4e98f31-5ffe-4ce1-be31-1b38b384c009 0 >nul 2>&1\npowercfg /setdcvalueindex scheme_current 2a737441-1930-4402-8d77-b2bebba308a3 d4e98f31-5ffe-4ce1-be31-1b38b384c009 0 >nul 2>&1\npowercfg /setacvalueindex scheme_current 2a737441-1930-4402-8d77-b2bebba308a3 48e6b7a6-50f5-4782-a5d4-53bb8f07e226 0 >nul 2>&1\npowercfg /setdcvalueindex scheme_current 2a737441-1930-4402-8d77-b2bebba308a3 48e6b7a6-50f5-4782-a5d4-53bb8f07e226 0 >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\USB\" /v \"DisableSelectiveSuspend\" /t REG_DWORD /d 1 /f >nul 2>&1\npowercfg /setactive scheme_current >nul 2>&1\necho [OK] USB idle and selective suspend fully disabled"
  },
  {
    "CategoryId": "input",
    "Category": "INPUT / MOUSE",
    "Id": "usb_controller_msi_priority",
    "Title": "USB Controller MSI + Thread Priority",
    "Description": "Enables MSI mode for USB controllers and sets USBHUB3/USBXHCI thread priority.",
    "Command": ":: === USB Controller MSI + Thread Priority ===\npowershell -NoProfile -ExecutionPolicy Bypass -Command \"Get-PnpDevice -Class USB -ErrorAction SilentlyContinue | Where-Object { $_.InstanceId -match '^PCI' } | ForEach-Object {   $regPath = 'HKLM:\\SYSTEM\\CurrentControlSet\\Enum\\' + $_.InstanceId + '\\Device Parameters\\Interrupt Management\\MessageSignaledInterruptProperties';   try {     if (-not (Test-Path $regPath)) { New-Item -Path $regPath -Force -ErrorAction SilentlyContinue | Out-Null };     Set-ItemProperty -Path $regPath -Name 'MSISupported' -Value 1 -Type DWord -Force -ErrorAction Stop;     Write-Host '[OK] MSI enabled for' $_.FriendlyName   } catch { Write-Host '[WARN] Skipped' $_.FriendlyName $_.Exception.Message } }\"\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\usbxhci\\Parameters\" /v ThreadPriority /t REG_DWORD /d 31 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\USBHUB3\\Parameters\" /v ThreadPriority /t REG_DWORD /d 31 /f >nul 2>&1\necho [OK] USB controller MSI and thread priority applied"
  },
  {
    "CategoryId": "gpu",
    "Category": "GPU / NVIDIA",
    "Id": "nvidia_force_p0_state",
    "Title": "NVIDIA Force P0 State",
    "Description": "Sets DisableDynamicPstate for NVIDIA display class keys. Based on your Force P0-State script.",
    "Command": ":: === NVIDIA Force P0 State ===\npowershell -NoProfile -ExecutionPolicy Bypass -Command \"Get-ItemProperty 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}\\*' -ErrorAction SilentlyContinue | Where-Object { $_.ProviderName -match 'NVIDIA' -or $_.DriverDesc -match 'NVIDIA' } | ForEach-Object {   $path = $_.PSPath;   try { Set-ItemProperty -Path $path -Name 'DisableDynamicPstate' -Value 1 -Type DWord -Force -ErrorAction Stop;   Write-Host '[OK] Set DisableDynamicPstate on' $path }   catch { Write-Host '[WARN] Could not set on' $path $_.Exception.Message } }\"\necho [OK] NVIDIA P0 state pass complete"
  },
  {
    "CategoryId": "gpu",
    "Category": "GPU / NVIDIA",
    "Id": "nvidia_smi_ecc_off",
    "Title": "NVIDIA ECC Off (If Supported)",
    "Description": "Runs nvidia-smi -e 0 only if nvidia-smi exists. Safe no-op on unsupported GPUs.",
    "Command": ":: === NVIDIA ECC Off If Supported ===\nif exist \"%ProgramFiles%\\NVIDIA Corporation\\NVSMI\\nvidia-smi.exe\" (\n  \"%ProgramFiles%\\NVIDIA Corporation\\NVSMI\\nvidia-smi.exe\" -e 0 >nul 2>&1\n  echo [OK] NVIDIA ECC command sent if supported\n) else (\n  echo [OK] nvidia-smi not found; ECC tweak skipped\n)"
  },
  {
    "CategoryId": "gpu",
    "Category": "GPU / NVIDIA",
    "Id": "nvidia_unrestricted_clock_policy",
    "Title": "NVIDIA Unrestricted Clock Policy",
    "Description": "Runs nvidia-smi -acp 0 if available. Safe no-op if unsupported or nvidia-smi missing.",
    "Command": ":: === NVIDIA Unrestricted Clock Policy ===\nif exist \"%ProgramFiles%\\NVIDIA Corporation\\NVSMI\\nvidia-smi.exe\" (\n  \"%ProgramFiles%\\NVIDIA Corporation\\NVSMI\\nvidia-smi.exe\" -acp 0 >nul 2>&1\n  echo [OK] NVIDIA clock permission command sent if supported\n) else (\n  echo [OK] nvidia-smi not found; clock policy tweak skipped\n)"
  },
  {
    "CategoryId": "gpu",
    "Category": "GPU / NVIDIA",
    "Id": "gpu_hags_gamemode_pack",
    "Title": "HAGS + Game Mode Pack",
    "Description": "Enables HAGS and Game Mode while keeping capture disabled.",
    "Command": ":: === HAGS + Game Mode Pack ===\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers\" /v HwSchMode /t REG_DWORD /d 2 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\GameBar\" /v AutoGameModeEnabled /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_Enabled /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\GameDVR\" /v AppCaptureEnabled /t REG_DWORD /d 0 /f >nul 2>&1\necho [OK] HAGS/Game Mode pack applied"
  },
  {
    "CategoryId": "gpu",
    "Category": "GPU / NVIDIA",
    "Id": "nvidia_telemetry_cleanup",
    "Title": "NVIDIA Telemetry Cleanup",
    "Description": "Disables common NVIDIA telemetry tasks/services when present. Missing items are ignored.",
    "Command": ":: === NVIDIA Telemetry Cleanup ===\nfor %%S in (NvTelemetryContainer NvContainerNetworkService) do (sc stop %%S >nul 2>&1 & sc config %%S start= disabled >nul 2>&1)\nfor %%T in (NvTmMon NvTmRep NvTmRepOnLogon) do schtasks /Change /TN \"\\NVIDIA\\%%T\" /Disable >nul 2>&1\nreg delete \"HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run\" /v NvBackend /f >nul 2>&1\necho [OK] NVIDIA telemetry cleanup completed"
  },
  {
    "CategoryId": "fortnite",
    "Category": "FORTNITE FPS / PERFORMANCE",
    "Id": "fse_safe_fullscreen_pack",
    "Title": "FSE Safe Fullscreen Pack",
    "Description": "Applies your FSE fullscreen values without fighting the GameDVR/FSE conflict fix.",
    "Command": ":: === FSE Safe Fullscreen Pack ===\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_FSEBehaviorMode /t REG_DWORD /d 2 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_FSEBehavior /t REG_DWORD /d 2 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_DSEBehavior /t REG_DWORD /d 2 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_DXGIHonorFSEWindowsCompatible /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_HonorUserFSEBehaviorMode /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\System\\GameConfigStore\" /v GameDVR_EFSEFeatureFlags /t REG_DWORD /d 0 /f >nul 2>&1\necho [OK] FSE safe fullscreen values applied"
  },
  {
    "CategoryId": "power",
    "Category": "POWER PLAN",
    "Id": "timer_power_gaming_safe",
    "Title": "Timer Power Gaming Safe",
    "Description": "Creates/enables Ultimate Performance and disables AC sleep/hibernate without permanent HPET/BCD flags.",
    "Command": ":: === Timer Power Gaming Safe ===\npowercfg -duplicatescheme e9a42b02-d5df-448d-aa00-03f14749eb61 >nul 2>&1\nfor /f \"tokens=4\" %%A in ('powercfg -list ^| findstr /i \"Ultimate\"') do powercfg -setactive %%A >nul 2>&1\npowercfg /change monitor-timeout-ac 0 >nul 2>&1\npowercfg /change standby-timeout-ac 0 >nul 2>&1\npowercfg /hibernate off >nul 2>&1\nbcdedit /set useplatformtick yes >nul 2>&1\nbcdedit /set disabledynamictick yes >nul 2>&1\nbcdedit /set tscsyncpolicy Enhanced >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Kernel\" /v \"GlobalTimerResolutionRequests\" /t REG_DWORD /d 1 /f >nul 2>&1\necho [OK] Timer/Power gaming safe preset applied"
  },
  {
    "CategoryId": "power",
    "Category": "POWER PLAN",
    "Id": "aggressive_power_plan_pack",
    "Title": "Aggressive Power Plan Pack",
    "Description": "Applies CPU 100%, boost policy, idle promote/demote, no sleep, USB/device idle off. Reboot recommended.",
    "Command": ":: === Aggressive Power Plan Pack ===\npowercfg /setacvalueindex scheme_current sub_processor PROCTHROTTLEMIN 100 >nul 2>&1\npowercfg /setacvalueindex scheme_current sub_processor PROCTHROTTLEMAX 100 >nul 2>&1\npowercfg /setdcvalueindex scheme_current sub_processor PROCTHROTTLEMIN 100 >nul 2>&1\npowercfg /setdcvalueindex scheme_current sub_processor PROCTHROTTLEMAX 100 >nul 2>&1\npowercfg /setacvalueindex scheme_current sub_processor CPMINCORES 100 >nul 2>&1\npowercfg /setacvalueindex scheme_current sub_processor CPMAXCORES 100 >nul 2>&1\npowercfg /setacvalueindex scheme_current sub_processor PERFEPP 0 >nul 2>&1\npowercfg /setacvalueindex scheme_current sub_processor PERFBOOSTPOL 100 >nul 2>&1\npowercfg /setacvalueindex scheme_current sub_processor PERFBOOSTMODE 2 >nul 2>&1\npowercfg /setacvalueindex scheme_current sub_processor IDLEDISABLE 1 >nul 2>&1\npowercfg /setacvalueindex scheme_current SUB_SLEEP AWAYMODE 0 >nul 2>&1\npowercfg /setacvalueindex scheme_current SUB_SLEEP ALLOWSTANDBY 0 >nul 2>&1\npowercfg /setacvalueindex scheme_current SUB_SLEEP HYBRIDSLEEP 0 >nul 2>&1\npowercfg /setacvalueindex scheme_current SUB_PCIEXPRESS ASPM 0 >nul 2>&1\npowercfg /setactive scheme_current >nul 2>&1\necho [OK] Aggressive power plan pack applied"
  },
  {
    "CategoryId": "network",
    "Category": "NETWORK OPTIMIZATION",
    "Id": "wifi_services_repair",
    "Title": "Wi-Fi Services Repair",
    "Description": "Re-enables core Wi-Fi/network services and active probing if networking breaks after extreme debloat.",
    "Command": ":: === Wi-Fi Services Repair ===\nfor %%S in (Dhcp DPS lmhosts NlaSvc nsi RmSvc Wcmsvc WlanSvc EventLog) do sc config %%S start= auto >nul 2>&1\nfor %%S in (Netman WwanSvc WinHttpAutoProxySvc LanmanWorkstation) do sc config %%S start= demand >nul 2>&1\nreg add \"HKLM\\Software\\Policies\\Microsoft\\Windows\\NetworkConnectivityStatusIndicator\" /v NoActiveProbe /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\System\\CurrentControlSet\\Services\\NlaSvc\\Parameters\\Internet\" /v EnableActiveProbing /t REG_DWORD /d 1 /f >nul 2>&1\nipconfig /flushdns >nul 2>&1\necho [OK] Wi-Fi/network services repair applied"
  },
  {
    "CategoryId": "registry",
    "Category": "REGISTRY TWEAKS",
    "Id": "windows_latency_registry_pack",
    "Title": "Windows Latency Registry Pack",
    "Description": "Adds selected safer latency keys from your windows.reg/kbm pack. Avoids CSRSS and risky processor C-state keys.",
    "Command": ":: === Windows Latency Registry Pack ===\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\PriorityControl\" /v Win32PrioritySeparation /t REG_DWORD /d 26 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\services\\mouclass\\Parameters\" /v MouseDataQueueSize /t REG_DWORD /d 18 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games\" /v \"GPU Priority\" /t REG_DWORD /d 8 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games\" /v Priority /t REG_DWORD /d 6 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games\" /v \"Scheduling Category\" /t REG_SZ /d High /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games\" /v \"SFIO Priority\" /t REG_SZ /d High /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\Psched\" /v NonBestEffortLimit /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\Power\\PowerThrottling\" /v PowerThrottlingOff /t REG_DWORD /d 1 /f >nul 2>&1\necho [OK] Windows latency registry pack applied"
  },
  {
    "CategoryId": "registry",
    "Category": "REGISTRY TWEAKS",
    "Id": "ntfs_8dot3_lastaccess",
    "Title": "NTFS Optimise — Last Access + 8.3 Names Off",
    "Description": "Disables NTFS last-access timestamp updates on every file read (reduces SSD write amplification during gaming) and disables 8.3 short filename creation (reduces filesystem overhead on every file create/read).",
    "Command": ":: === NTFS Last Access + 8dot3 Off ===\nfsutil behavior set disableLastAccess 1 >nul 2>&1\nfsutil behavior set disable8dot3 1 >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\FileSystem\" /v \"NtfsDisable8dot3NameCreation\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\FileSystem\" /v \"NtfsDisableLastAccessUpdate\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\FileSystem\" /v \"NtfsEncryptionService\" /t REG_DWORD /d 0 /f >nul 2>&1\necho [OK] NTFS last access and 8.3 name creation disabled"
  },
  {
    "CategoryId": "registry",
    "Category": "REGISTRY TWEAKS",
    "Id": "disable_cortana_recall_copilot",
    "Title": "Disable Cortana / Copilot / Recall / Bing",
    "Description": "Disables Cortana, Windows Copilot, Windows Recall, and Bing search integration. All three run background AI processes that consume CPU and network bandwidth during gameplay.",
    "Command": ":: === Disable Cortana / Copilot / Recall / Bing ===\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\Windows Search\" /v \"AllowCortana\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Search\" /v \"BingSearchEnabled\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Search\" /v \"AllowSearchToUseLocation\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Search\" /v \"CortanaConsent\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\WindowsAI\" /v \"DisableAIDataAnalysis\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\Software\\Policies\\Microsoft\\Windows\\WindowsAI\" /v \"DisableAIDataAnalysis\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\WindowsCopilot\" /v \"TurnOffWindowsCopilot\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\Software\\Policies\\Microsoft\\Windows\\WindowsCopilot\" /v \"TurnOffWindowsCopilot\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Advanced\" /v \"ShowCopilotButton\" /t REG_DWORD /d 0 /f >nul 2>&1\ntaskkill /f /im Copilot.exe >nul 2>&1\ntaskkill /f /im AIHost.exe >nul 2>&1\necho [OK] Cortana, Copilot, Recall, and Bing disabled"
  },
  {
    "CategoryId": "registry",
    "Category": "REGISTRY TWEAKS",
    "Id": "disable_spotlight_lockscreen",
    "Title": "Disable Windows Spotlight / Lock Screen Ads",
    "Description": "Stops Windows Spotlight from downloading and rotating lock screen ads/images. Eliminates background network traffic and CPU usage for content fetching that happens even during gaming sessions.",
    "Command": ":: === Disable Spotlight / Lock Screen Ads ===\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager\" /v \"RotatingLockScreenEnabled\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager\" /v \"RotatingLockScreenOverlayEnabled\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager\" /v \"SubscribedContent-338387Enabled\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager\" /v \"SubscribedContent-338388Enabled\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager\" /v \"SubscribedContent-338389Enabled\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager\" /v \"SubscribedContent-353698Enabled\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager\" /v \"SoftLandingEnabled\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager\" /v \"FeatureManagementEnabled\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\CloudContent\" /v \"DisableWindowsSpotlightFeatures\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\CloudContent\" /v \"DisableWindowsConsumerFeatures\" /t REG_DWORD /d 1 /f >nul 2>&1\necho [OK] Windows Spotlight and lock screen ads disabled"
  },
  {
    "CategoryId": "registry",
    "Category": "REGISTRY TWEAKS",
    ...
    "Description": "Stops Windows from compressing inactive memory pages in RAM. On gaming PCs with 16GB+ RAM this reduces CPU overhead — the compression/decompression cycles waste CPU time that could go to Fortnite.",
    "Command": ":: === Disable Memory Compression ===\npowershell -NoProfile -ExecutionPolicy Bypass -Command \"try { Disable-MMAgent -MemoryCompression -ErrorAction Stop; Write-Host '[OK] Memory compression disabled' } catch { Write-Host '[WARN] Could not disable memory compression:' $_.Exception.Message }\"\necho [OK] Memory compression pass complete"
  },
  {
    "CategoryId": "registry",
    "Category": "REGISTRY TWEAKS",
    "Id": "disable_waasmedicasvc_medic",
    "Title": "Block Windows Update Medic (WaaSMedicSvc)",
    "Description": "WaaSMedicSvc re-enables Windows Update even after you disable it. This takes ownership of its registry Start key and sets it to disabled, preventing it from self-healing. Requires admin.",
    "Command": ":: === Block WaaSMedicSvc Self-Healing ===\n:: Take ownership of the WaaSMedicSvc Start key\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\WaaSMedicSvc\" /v \"Start\" /t REG_DWORD /d 4 /f >nul 2>&1\npowershell -NoProfile -ExecutionPolicy Bypass -Command \"\n$svc = 'HKLM:\\SYSTEM\\CurrentControlSet\\Services\\WaaSMedicSvc';\n$acl = Get-Acl $svc;\n$owner = [System.Security.Principal.NTAccount]'Administrators';\ntry { $acl.SetOwner($owner) } catch {};\n$rule = [System.Security.AccessControl.RegistryAccessRule]::new('Administrators','FullControl','Allow');\ntry { $acl.SetAccessRule($rule); Set-Acl $svc $acl } catch {};\ntry { Set-ItemProperty -Path $svc -Name 'Start' -Value 4 -Type DWord -Force } catch {};\nWrite-Host '[OK] WaaSMedicSvc ownership and Start=4 set'\"\nsc stop WaaSMedicSvc >nul 2>&1\necho [OK] WaaSMedicSvc blocked"
  },
  {
    "CategoryId": "network",
    "Category": "NETWORK",
    "Id": "cloudflare_dns",
    "Title": "Set DNS to Cloudflare 1.1.1.1 (All Adapters)",
    "Description": "Sets primary DNS to 1.1.1.1 and secondary to 1.0.0.1 (Cloudflare) on every active network adapter. Cloudflare DNS is consistently the fastest global DNS resolver — reduces game server connection latency.",
    "Command": ":: === Set Cloudflare DNS on All Adapters ===\npowershell -NoProfile -ExecutionPolicy Bypass -Command \"\nGet-NetAdapter -Physical -ErrorAction SilentlyContinue | Where-Object { $_.Status -eq 'Up' } | ForEach-Object {\n    try {\n        Set-DnsClientServerAddress -InterfaceIndex $_.InterfaceIndex -ServerAddresses @('1.1.1.1','1.0.0.1') -ErrorAction Stop\n        Write-Host '[OK] DNS set on' $_.Name\n    } catch { Write-Host '[WARN] Could not set DNS on' $_.Name $_.Exception.Message }\n}\"\nipconfig /flushdns >nul 2>&1\necho [OK] Cloudflare DNS applied to all active adapters"
  },
  {
    "CategoryId": "network",
    "Category": "NETWORK",
    "Id": "disable_ipv6_full",
    "Title": "Disable IPv6 — Registry + All Adapters",
    "Description": "Disables IPv6 both in registry (DisabledComponents=255) AND via netsh on every adapter. Registry-only method sometimes doesn't fully disable IPv6 on modern Windows — this does both.",
    "Command": ":: === Disable IPv6 Fully ===\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\Tcpip6\\Parameters\" /v \"DisabledComponents\" /t REG_DWORD /d 255 /f >nul 2>&1\npowershell -NoProfile -ExecutionPolicy Bypass -Command \"\nGet-NetAdapter -ErrorAction SilentlyContinue | ForEach-Object {\n    try { Disable-NetAdapterBinding -Name $_.Name -ComponentID ms_tcpip6 -ErrorAction SilentlyContinue; Write-Host '[OK] IPv6 disabled on' $_.Name }\n    catch { Write-Host '[WARN] Skipped' $_.Name }\n}\"\necho [OK] IPv6 disabled via registry and all adapters"
  },
  {
    "CategoryId": "network",
    "Category": "NETWORK",
    "Id": "full_duplex_adapters",
    "Title": "Force Full Duplex on All Network Adapters",
    "Description": "Forces all physical network adapters to Full Duplex mode. Auto-negotiation can sometimes settle on half duplex, halving effective bandwidth and causing jitter. Forcing Full Duplex prevents this.",
    "Command": ":: === Force Full Duplex on All Adapters ===\npowershell -NoProfile -ExecutionPolicy Bypass -Command \"\nGet-NetAdapter -Physical -ErrorAction SilentlyContinue | ForEach-Object {\n    $n = $_.Name\n    Get-NetAdapterAdvancedProperty -Name $n -ErrorAction SilentlyContinue |\n    Where-Object { $_.DisplayName -match 'Speed.*Duplex|Duplex|Link Speed' } |\n    ForEach-Object {\n        $vals = $_.ValidDisplayValues\n        $fullDuplex = $vals | Where-Object { $_ -match '(1.*Gbps|100.*Mbps).*Full' } | Select-Object -First 1\n        if ($fullDuplex) {\n            try { Set-NetAdapterAdvancedProperty -Name $n -DisplayName $_.DisplayName -DisplayValue $fullDuplex -NoRestart -ErrorAction Stop; Write-Host '[OK] Full duplex set on' $n }\n            catch { Write-Host '[WARN] Could not set on' $n }\n        }\n    }\n}\"\necho [OK] Full duplex pass complete"
  },
  {
    "CategoryId": "power",
    "Category": "POWER PLAN",
    "Id": "bcd_boot_optimize",
    "Title": "BCDEdit Boot Optimisations",
    "Description": "Disables dynamic tick (reduces timer interrupts), forces platform clock, sets TSC sync to Enhanced for consistent CPU timing, disables boot status policy so PC boots faster after crashes. All safe gaming BCDEdit tweaks.",
    "Command": ":: === BCDEdit Boot Optimisations ===\nbcdedit /set useplatformtick yes >nul 2>&1\nbcdedit /set disabledynamictick yes >nul 2>&1\nbcdedit /set tscsyncpolicy Enhanced >nul 2>&1\nbcdedit /set bootstatuspolicy ignoreallfailures >nul 2>&1\nbcdedit /set quietboot yes >nul 2>&1\nbcdedit /set nx OptIn >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Kernel\" /v \"GlobalTimerResolutionRequests\" /t REG_DWORD /d 1 /f >nul 2>&1\necho [OK] BCDEdit boot optimisations applied"
  },
  {
    "CategoryId": "power",
    "Category": "POWER PLAN",
    "Id": "disable_sleep_study",
    "Title": "Disable SleepStudy / Energy Reports",
    "Description": "Disables the SleepStudy background ETW logger that continuously tracks power events and writes energy reports. Eliminates a persistent background ETW logging overhead that runs even during gameplay.",
    "Command": ":: === Disable SleepStudy / Energy Reports ===\npowercfg /sleepstudy /output nul >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\Power\" /v \"EnergyEstimationEnabled\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\Power\" /v \"EnergyEstimationDisabled\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\Power\\PowerSettings\\012ee47a-9e91-4b39-a0a6-3523e36d3a58\\0e796bdb-100d-47d6-a2d5-f7d2daa51f51\" /v \"Attributes\" /t REG_DWORD /d 2 /f >nul 2>&1\nschtasks /Change /TN \"\\Microsoft\\Windows\\Power Efficiency Diagnostics\\AnalyzeSystem\" /Disable >nul 2>&1\necho [OK] SleepStudy and energy reports disabled"
  },
  {
    "CategoryId": "fortnite",
    "Category": "FORTNITE FPS / PERFORMANCE",
    "Id": "fortnite_process_priority",
    "Title": "Set Fortnite Process to HIGH Priority",
    "Description": "Sets FortniteClient-Win64-Shipping.exe to HIGH CPU priority class and tells Windows to prefer it over all background tasks. Run this AFTER Fortnite is already open for max effect.",
    "Command": ":: === Set Fortnite to HIGH Priority ===\npowershell -NoProfile -ExecutionPolicy Bypass -Command \"\n$procs = @('FortniteClient-Win64-Shipping','FortniteLauncher','EpicGamesLauncher')\nforeach ($pname in $procs) {\n    $p = Get-Process -Name $pname -ErrorAction SilentlyContinue\n    if ($p) {\n        try { $p.PriorityClass = [System.Diagnostics.ProcessPriorityClass]::High; Write-Host '[OK] HIGH priority set on' $pname }\n        catch { Write-Host '[WARN] Could not set priority on' $pname }\n    } else { Write-Host '[INFO]' $pname 'not running - launch Fortnite first' }\n}\"\necho [OK] Fortnite process priority pass complete"
  },
  {
    "CategoryId": "gpu",
    "Category": "GPU / NVIDIA",
    "Id": "nvidia_preemption_tweak",
    "Title": "NVIDIA GPU Preemption Level (Low Latency)",
    "Description": "Sets NVIDIA compute preemption to thread-level and graphics preemption to DMA buffer level. Lower preemption granularity reduces GPU interrupt latency between frames in Fortnite.",
    "Command": ":: === NVIDIA GPU Preemption Tweak ===\npowershell -NoProfile -ExecutionPolicy Bypass -Command \"\nGet-ItemProperty 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}\\*' -ErrorAction SilentlyContinue |\nWhere-Object { $_.DriverDesc -match 'NVIDIA' -or $_.ProviderName -match 'NVIDIA' } |\nForEach-Object {\n    $p = $_.PSPath\n    try { Set-ItemProperty $p -Name 'ComputePreemption' -Value 1 -Type DWord -Force -ErrorAction SilentlyContinue } catch {}\n    try { Set-ItemProperty $p -Name 'GraphicsPreemption' -Value 2 -Type DWord -Force -ErrorAction SilentlyContinue } catch {}\n    try { Set-ItemProperty $p -Name 'D3PCLatency' -Value 1 -Type DWord -Force -ErrorAction SilentlyContinue } catch {}\n    Write-Host '[OK] NVIDIA preemption set on' $_.DriverDesc\n}\"\necho [OK] NVIDIA GPU preemption tweak applied"
  },
  {
    "CategoryId": "debloat",
    "Category": "PROCESS DEBLOAT",
    "Id": "disable_driver_auto_install",
    "Title": "Disable Automatic Driver Installation",
    "Description": "Stops Windows from automatically downloading and installing drivers in the background. Driver installs mid-session can cause stutters, GPU resets, and FPS drops. You manage driver updates manually.",
    "Command": ":: === Disable Automatic Driver Installation ===\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DriverSearching\" /v \"SearchOrderConfig\" /t REG_DWORD /d 0 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Device Installer\" /v \"DisableCoInstallers\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\WindowsUpdate\" /v \"ExcludeWUDriversInQualityUpdate\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\Device Metadata\" /v \"PreventDeviceMetadataFromNetwork\" /t REG_DWORD /d 1 /f >nul 2>&1\nreg add \"HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DriverSearching\" /v \"DontSearchWindowsUpdate\" /t REG_DWORD /d 1 /f >nul 2>&1\necho [OK] Automatic driver installation disabled"
  },
  {
    "CategoryId": "debloat",
    "Category": "PROCESS DEBLOAT",
    "Id": "flush_gpu_shader_cache",
    "Title": "Flush / Reset GPU Shader Cache",
    "Description": "Clears stale DX shader caches, NVIDIA shader cache, and DirectX compiler cache. Run this before a gaming session if you have stutters on first loading new areas — forces a clean recompile.",
    "Command": ":: === Flush GPU Shader Cache ===\n:: Clear DirectX Shader Cache\nfor /d %%D in (\"%LocalAppData%\\D3DSCache\\*\") do rd /s /q \"%%D\" >nul 2>&1\ndel /f /q \"%LocalAppData%\\D3DSCache\\*\" >nul 2>&1\n:: Clear NVIDIA GLCache\nfor /d %%D in (\"%LocalAppData%\\NVIDIA\\GLCache\\*\") do rd /s /q \"%%D\" >nul 2>&1\ndel /f /q \"%LocalAppData%\\NVIDIA\\GLCache\\*\" >nul 2>&1\n:: Clear AMD shader cache\nfor /d %%D in (\"%LocalAppData%\\AMD\\DxCache\\*\") do rd /s /q \"%%D\" >nul 2>&1\ndel /f /q \"%LocalAppData%\\AMD\\DxCache\\*\" >nul 2>&1\n:: Clear DXCache folder\nfor /d %%D in (\"%LocalAppData%\\Microsoft\\Windows\\DirectX12\\UMD\\*\") do rd /s /q \"%%D\" >nul 2>&1\necho [OK] GPU shader cache flushed - shaders will recompile on next game launch"
  },
  {
    "CategoryId": "smart",
    "Category": "SMART TWEAKS",
    "Id": "smart_irp_stack_gaming",
    "Title": "Raise IRP Stack Size for Gaming",
    "Description": "Increases LanmanServer IRPStackSize from default 15 to 32. Prevents 'insufficient resources' errors in network I/O stacks during high-packet-rate gaming. Also sets server memory to BalancedMemoryUsage.",
    "Command": ":: === IRP Stack Size for Gaming ===\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\LanmanServer\\Parameters\" /v \"IRPStackSize\" /t REG_DWORD /d 32 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\LanmanServer\\Parameters\" /v \"MaxWorkItems\" /t REG_DWORD /d 8192 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\LanmanServer\\Parameters\" /v \"MaxMpxCt\" /t REG_DWORD /d 1024 /f >nul 2>&1\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\LanmanServer\\Parameters\" /v \"MaxRawWorkItems\" /t REG_DWORD /d 512 /f >nul 2>&1\necho [OK] IRP stack size and LanmanServer params tuned"
  }
]
'@ | ConvertFrom-Json
...
$script:LastSnapshotDir = $null
$script:UndoHistoryPath = Join-Path $script:SettingsDir 'last-undo-session.json'
$script:ApplyInProgress = $false
$script:PendingSelections = @{}
$script:StabilityModeIds = @("smart_atom_core_safe","smart_winaero_responsive_shell","smart_wpd_privacy_core","smart_safe_service_trim","smart_competitive_scheduler","smart_network_stability","pro_fse_gamedvr","pro_mmcss_scheduler","win32priority_26_clean","directx_swap_upgrade","gpu_hags_gamemode_pack","mouseaccel","mouse_1to1_sens","directinput_accel_off","usb_idle_power_saving_off","timer_power_gaming_safe")
$script:LastRamPct       = ""
$script:RamTickCount    = 0
$script:CpuCounterPrimed = $false
...
function Clear-TweakSelections {
    foreach ($entry in $script:CheckboxMap.Values) {
        if ($entry.CheckBox.IsEnabled) { $entry.CheckBox.IsChecked = $false }
    }
    $script:PendingSelections = @{}
    Update-SelectedCount
}

function Select-TweakById([string]$id) {
    if ($script:CheckboxMap.ContainsKey($id)) {
        $entry = $script:CheckboxMap[$id]
        if ($entry.CheckBox.IsEnabled) { $entry.CheckBox.IsChecked = $true }
    } else {
        # Tweak card isn't built yet (section not visible) - store as pending
        if (-not $script:PendingSelections) { $script:PendingSelections = @{} }
        $script:PendingSelections[$id] = $true
    }
}
...
function Select-Preset([string]$Mode) {
    ...
    switch ($Mode) {
        'Safe' {
            $safeCats = @('smart','privacy','debloat','registry')
        ...
        'Competitive' {
            $compCats = @('smart','fortnite','network','input','power','gpu')
            $selectedIds = @($script:Catalog | Where-Object {
...
function New-LakRestorePoint {
    try {
        Write-Log 'Creating restore point: LakTweaks v62 Before Apply...' 'INFO'
        Checkpoint-Computer -Description 'LakTweaks v62 Before Apply' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
...
function Initialize-FastPerformanceCounters {
    ...
    try {
        ...
        [void]$script:CpuCounter.NextValue() # prime first zero sample
    } catch { $script:CpuCounter = $null; $script:LastCpuPct = 'N/A' }
    try {
        $script:RamCounter = [System.Diagnostics.PerformanceCounter]::new('Memory','Available MBytes')
    } catch { $script:RamCounter = $null }
    try {
        $osObj = Get-CimInstance Win32_OperatingSystem -ErrorAction Stop
        $script:TotalRamMB = [double]($osObj.TotalVisibleMemorySize / 1024)
        if ($script:RamCounter) {
            $avail = [double]$script:RamCounter.NextValue()
            $usedPct = [math]::Round((( $script:TotalRamMB - $avail ) / $script:TotalRamMB) * 100, 0)
            $script:LastRamPct = $usedPct.ToString() + '%'
        }
    } catch { $script:TotalRamMB = 0; if (-not $script:LastRamPct) { $script:LastRamPct = 'N/A' } }
}

function Refresh-LiveStatsCache {
    try {
        if ($script:CpuCounter) {
            $rawCpu = [math]::Round([double]$script:CpuCounter.NextValue(), 0)
            if ($rawCpu -lt 0) { $rawCpu = 0 }
            if ($rawCpu -gt 100) { $rawCpu = 100 }
            $script:LastCpuPct = $rawCpu.ToString() + '%'
        }
    } catch { $script:LastCpuPct = 'N/A' }

    try {
        if ($script:RamCounter -and $script:TotalRamMB -gt 0) {
            $avail = [double]$script:RamCounter.NextValue()
            $usedPct = [math]::Round((( $script:TotalRamMB - $avail ) / $script:TotalRamMB) * 100, 0)
    ...
    } catch { $script:LastRamPct = 'N/A' }
}

function Update-PerformancePanel {
    # Ultra-light UI update. No CIM/WMI here.
    try {
        ...
        if ($null -ne $CpuValueText -and $script:LastCpuPct) { $CpuValueText.Text = $script:LastCpuPct }
        if ($null -ne $RamValueText -and $script:LastRamPct) { $RamValueText.Text = $script:LastRamPct }
    } catch {}
}

function Start-LakStatsTimer {
    if ($script:StatsTimer) {
    ...
    $timer.Add_Tick({
        try {
            $script:StatsTick++
            # Time updates every second. CPU/RAM cache refreshes every 2 seconds to stay smooth.
            if (($script:StatsTick % 2) -eq 0) { Refresh-LiveStatsCache }
            Update-PerformancePanel
        } catch {}
    })
    $script:StatsTimer = $timer
    $timer.Start()
}
...
function Backup-LakFullRegistry {
    try {
        ...
        [System.Windows.MessageBox]::Show('Full registry backup saved to:`n' + $dir, 'LakTweaks v62', 'OK', 'Information') | Out-Null
...
function Restore-LakFullRegistry {
    try {
        ...
        $r = [System.Windows.MessageBox]::Show('Restore the saved full registry backup now? You should reboot after restoring.', 'LakTweaks v62', 'YesNo', 'Warning')
        if ($r -ne 'Yes') { return }
...
function Select-SmartOptimizePreset {
    ...
    $ids = @($script:Catalog | Where-Object {
        ($_.CategoryId -in @('smart','network','input','power','gpu','privacy','registry','debloat')) -and
...
function Select-AutoOptimizerPreset {
    # Clear all current selections including pending
    Clear-TweakSelections

    $hw = Get-LakHardwareSummary
    Write-Log ('Auto Optimizer scanning: ' + $hw.OS + ' | ' + $hw.CPU + ' | ' + $hw.GPU + ' | ' + $hw.RAM + ' | Network=' + $hw.Network) 'INFO'

    $baseCategories = @('smart','input','network','power','gpu','privacy','registry','debloat','fortnite')
    $denyPattern    = 'extreme|strip|defender|wupdate|tdr|spectre|cstates|trusted|eventlog|samss|full_service|oneclick_full|disable_windows_update|windows_update|hidsvc|hidserv|nlasvc|lanman|gameinput|fortnite_settings_note|largesystemcache|memory manager|csrss'

    $ids = @($script:Catalog | Where-Object {
        ($_.CategoryId -in $baseCategories) -and
        ($_.Id -notmatch $denyPattern) -and
        ($_.Title -notmatch 'Extreme|OneClick Full|Defender|Windows Update|Reminder')
    } | Select-Object -ExpandProperty Id -Unique)

    if ($hw.GPU -notmatch 'NVIDIA') {
        $ids = @($ids | Where-Object { $_ -notmatch 'nvidia|msi_latency|shader|nv_' })
        Write-Log 'Auto Optimizer: NVIDIA tweaks skipped (non-NVIDIA GPU detected).' 'INFO'
    }
    if ($hw.Network -match 'Wi-?Fi|Wireless') {
        $ids = @($ids | Where-Object { $_ -notmatch 'nic_interrupt|green_ethernet|eee' })
        Write-Log 'Auto Optimizer: Ethernet-only NIC tweaks skipped (Wi-Fi detected).' 'INFO'
    }

    # Store ALL selected ids as pending - they'll be applied regardless of which section is visible
    foreach ($id in $ids) {
        if (-not $script:PendingSelections) { $script:PendingSelections = @{} }
        $script:PendingSelections[$id] = $true
        # Also set in CheckboxMap if this section is currently visible
        Select-TweakById $id
    }

    Update-SelectedCount
    Write-Log ('Auto Optimizer queued ' + $ids.Count + ' conflict-safe tweaks. Click APPLY SELECTED on any section or the Home page.') 'PRESET'
}


function Select-StabilityMode {
    Clear-TweakSelections
    foreach($id in $script:StabilityModeIds) { Select-TweakById $id }
    Update-SelectedCount
    Write-Log ("FPS Stability Mode queued " + $script:StabilityModeIds.Count + " conflict-safe tweaks. Click APPLY SELECTED.") "PRESET"
}
...
function Set-ApplyProgress([int]$current, [int]$total, [string]$status) {
    try {
        $pct = if ($total -gt 0) { [math]::Round(($current / [double]$total) * 100, 0) } else { 0 }
        if ($ApplyProgressBar) { $ApplyProgressBar.Maximum = 100; $ApplyProgressBar.Value = [double]$pct }
        if ($ApplyProgressText) { $ApplyProgressText.Text = "$status  ($current/$total - $pct%)" }
        Write-Log $status 'INFO'
        Invoke-UiPulse
    } catch {}
}

function New-LakStateSnapshot([object[]]$selected) {
    try {
        $root = Join-Path $script:SettingsDir 'Snapshots'
        if (-not (Test-Path $root)) { New-Item -ItemType Directory -Force -Path $root | Out-Null }
        $dir = Join-Path $root (Get-Date -Format 'yyyyMMdd_HHmmss')
        New-Item -ItemType Directory -Force -Path $dir | Out-Null
        $script:LastSnapshotDir = $dir
        [pscustomobject]@{ Version='v62 PRO+'; Time=(Get-Date).ToString('s'); Count=$selected.Count; Tweaks=@($selected | Select-Object Id,Title,CategoryId) } | ConvertTo-Json -Depth 5 | Set-Content -Path (Join-Path $dir 'session.json') -Encoding UTF8
        foreach($r in @(
            @{Path='HKCU\Control Panel\Mouse'; File='HKCU_Mouse.reg'},
            @{Path='HKCU\System\GameConfigStore'; File='HKCU_GameConfigStore.reg'},
            @{Path='HKCU\Software\Microsoft\Windows\CurrentVersion\GameDVR'; File='HKCU_GameDVR.reg'},
            @{Path='HKLM\SYSTEM\CurrentControlSet\Control\GraphicsDrivers'; File='HKLM_GraphicsDrivers.reg'},
            @{Path='HKLM\SYSTEM\CurrentControlSet\Control\PriorityControl'; File='HKLM_PriorityControl.reg'},
            @{Path='HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile'; File='HKLM_SystemProfile.reg'},
            @{Path='HKLM\SYSTEM\CurrentControlSet\Control\Power'; File='HKLM_Power.reg'}
        )) { cmd.exe /c ('reg export "' + $r.Path + '" "' + (Join-Path $dir $r.File) + '" /y >nul 2>&1') | Out-Null }
        cmd.exe /c ('powercfg /getactivescheme > "' + (Join-Path $dir 'active-power-plan.txt') + '" 2>nul') | Out-Null
        Write-Log ('Snapshot saved: ' + $dir) 'OK'
        return $dir
    } catch { Write-Log ('Snapshot failed: ' + $_.Exception.Message) 'WARN'; return $null }
}

function Restore-LakLastSnapshot {
    try {
        $root = Join-Path $script:SettingsDir 'Snapshots'
        if (-not (Test-Path $root)) { Write-Log 'No snapshot folder found.' 'WARN'; return }
        $last = Get-ChildItem -Path $root -Directory | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if (-not $last) { Write-Log 'No snapshots available.' 'WARN'; return }
        $r = [System.Windows.MessageBox]::Show('Restore last LakTweaks snapshot? Reboot after restoring.', 'LakTweaks v62', 'YesNo', 'Warning')
        if ($r -ne 'Yes') { return }
        Get-ChildItem -Path $last.FullName -Filter '*.reg' | ForEach-Object { cmd.exe /c ('reg import "' + $_.FullName + '" >nul 2>&1') | Out-Null }
        Write-Log ('Last snapshot restored: ' + $last.Name + '. Reboot recommended.') 'OK'
    } catch { Write-Log ('Snapshot restore failed: ' + $_.Exception.Message) 'FAIL' }
}

function Test-TweakAllowedByConflictEngine([object]$item) {
    $deny = 'eventlog|samss|trustedinstaller|dcomlaunch|plugplay|rpcss|nlasvc|lanmanworkstation|winmgmt|csrss|large_system_cache|largesystemcache'
    if ($item.Id -match $deny -or $item.Title -match 'Event Log|SAM|TrustedInstaller|CSRSS|LargeSystemCache') { return $false }
    return $true
}
function Invoke-TweakCommands([object[]]$selected) {
    ...
    foreach($item in $selected) {
        $i++
        if (-not (Test-TweakAllowedByConflictEngine $item)) {
            Write-Log ("Conflict engine blocked risky tweak: " + $item.Title) 'WARN'
            continue
        }
        Set-ApplyProgress $i $total ("Applying: " + $item.Title)
        $safeId = ([string]$item.Id) -replace '[^a-zA-Z0-9_-]', '_'
        $stamp = [guid]::NewGuid().ToString('N')
        $tempBat = Join-Path $env:TEMP ("LakTweaks_{0}_{1}.cmd" -f $safeId, $stamp)
        $tempLog = Join-Path $env:TEMP ("LakTweaks_{0}_{1}.log" -f $safeId, $stamp)
        try {
            if ($script:CheckboxMap.ContainsKey($item.Id)) { Set-TweakApplyingVisualState $item.Id }
            $cmdText = [string]$item.Command
            while ($cmdText.Contains('\\')) { $cmdText = $cmdText.Replace('\\','\') }
            $cmdText = $cmdText -replace "`r?`n", "`r`n"
            $batContent = @(
                '@echo off',
                'setlocal EnableExtensions EnableDelayedExpansion',
                'echo [LAKTWEAKS] Starting: ' + $item.Title,
                'echo [LAKTWEAKS] TweakId: ' + $item.Id,
                $cmdText,
            ...
            $psi.Arguments = '/d /c call "' + $tempBat + '"'
            ...
            try { [System.IO.File]::WriteAllText($tempLog, $output, [System.Text.UTF8Encoding]::new($false)) } catch {}
            $okMarker = ($output -match '\[OK\]')
            $hardFail = ($output -match '(?i)Access is denied|Invalid syntax|The syntax of the command is incorrect|not recognized as an internal|The system cannot find the path specified')
            if ($proc.ExitCode -eq 0 -or ($okMarker -and -not $hardFail)) {
                $verify = Verify-TweakPostApply $item
                $succeeded += $item
                if ($null -eq $verify) { Write-Log ($item.Title + ' applied. Verify: not required.') 'OK' }
                elseif ($verify) { Write-Log ($item.Title + ' applied. Verify passed.') 'OK' }
                else { Write-Log ($item.Title + ' applied. Verify partial/not confirmed.') 'WARN' }
            } else {
                $short = if ($output.Length -gt 260) { $output.Substring(0,260) + '...' } else { $output }
                $failed += [pscustomobject]@{ Item=$item; ExitCode=$proc.ExitCode; Output=$output }
                $script:LastFailedTweaks += $item
                Write-Log ($item.Title + ' failed. ExitCode=' + $proc.ExitCode + ' ' + $short) 'FAIL'
            }
        } catch {
    ...
    Set-ApplyProgress $total $total "Apply complete"
  ...
  <Grid>
    ...
    <Border Grid.Row="0" Grid.Column="0" Background="#050a0f" BorderBrush="#142b35" BorderThickness="0,0,1,1">
      <Grid VerticalAlignment="Center" HorizontalAlignment="Center" Width="34" Height="34">
        ...
        <TextBlock Text="L" FontSize="17" FontWeight="Black" Foreground="#00ffe7" HorizontalAlignment="Center" VerticalAlignment="Center" Margin="-6,-1,0,0"/>
        <TextBlock Text="T" FontSize="14" FontWeight="Black" Foreground="#ffffff" HorizontalAlignment="Center" VerticalAlignment="Center" Margin="8,6,0,0"/>
      </Grid>
    </Border>
      ...
      <Grid Margin="22,0,22,0">
        ...
        <StackPanel Orientation="Vertical" VerticalAlignment="Center">
          <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
            ...
            <TextBlock Text=" v62" FontSize="25" FontWeight="Black" Foreground="#00ffe7" Margin="14,0,0,0"/>
    ...
    <Grid x:Name="MainContentGrid" Grid.Row="1" Grid.Column="1" Margin="16,16,16,14">
      <Grid.RowDefinitions><RowDefinition x:Name="HomeRowDef" Height="216"/><RowDefinition x:Name="ContentRowDef" Height="*"/></Grid.RowDefinitions>

      <Border x:Name="HomeDashboardBorder" Grid.Row="0" CornerRadius="18" Background="#071018" BorderBrush="#183542" BorderThickness="1.2" Padding="14,10">
        <Grid>
          <Grid.RowDefinitions>
          ...
          <!-- ROW 1: perfectly equal preset/utility buttons -->
          <UniformGrid Grid.Row="0" Rows="1" Columns="9" HorizontalAlignment="Stretch" VerticalAlignment="Stretch">
            <Button x:Name="SafePresetBtn"    Margin="5,0" Height="62" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="CompPresetBtn"    Margin="5,0" Height="62" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="ExtremePresetBtn" Margin="5,0" Height="62" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="SelectAllBtn"     Margin="5,0" Height="62" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="ResetAllBtn"      Margin="5,0" Height="62" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="ExportBatBtn"     Margin="5,0" Height="62" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="RestorePointBtn"  Margin="5,0" Height="62" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="RetryFailedBtn"   Margin="5,0" Height="62" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="ClearLogBtn"      Margin="5,0" Height="62" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
          </UniformGrid>

          <!-- ROW 2: fixed grid so Apply + Selected can never get clipped -->
          <Grid Grid.Row="2" HorizontalAlignment="Stretch" VerticalAlignment="Stretch">
            ...
            <Button x:Name="OptimizeBtn"      Grid.Column="0" Margin="5,0" Height="52" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="LowTimerBtn"      Grid.Column="1" Margin="5,0" Height="52" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="SmartNetBtn"      Grid.Column="2" Margin="5,0" Height="52" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="FullBackupBtn"    Grid.Column="3" Margin="5,0" Height="52" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="RestoreBackupBtn" Grid.Column="4" Margin="5,0" Height="52" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="RamCleanBtn"      Grid.Column="5" Margin="5,0" Height="52" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="ApplyBtn"         Grid.Column="6" Margin="5,0" Height="52" FontSize="9" FontWeight="Black" Background="#071f14" Foreground="#00ffe7"/>
            <Border Grid.Column="7" Margin="7,0,0,0" CornerRadius="10" Background="#090f15" BorderBrush="#243b45" BorderThickness="1" Padding="4,2">
              <StackPanel VerticalAlignment="Center" HorizontalAlignment="Center">
                <TextBlock Text="SELECTED" FontSize="9" Foreground="#bcecff" FontWeight="Bold" HorizontalAlignment="Center"/>
                <TextBlock x:Name="SelectedCountText" Text="0" FontSize="24" FontWeight="Black" Foreground="#ffaa2b" HorizontalAlignment="Center"/>
              </StackPanel>
            </Border>
          </Grid>

          <!-- compact Home-only log row -->
          <Border Grid.Row="4" CornerRadius="9" BorderThickness="1" BorderBrush="#163847" Background="#03080d" Padding="8,2">
            <Grid>
      ...
      <Border Grid.Row="1" CornerRadius="16" Background="#050b10" BorderBrush="#142b35" BorderThickness="1.2" Margin="0,14,0,0" Padding="18,14">
        <Grid>
          ...
          <Border x:Name="SectionApplyBar" Grid.Row="2" Visibility="Collapsed" Margin="0,14,0,0" Padding="14,10" CornerRadius="12" Background="#071f14" BorderBrush="#2ed573" BorderThickness="1.1">
            <Grid>
              <Grid.ColumnDefinitions><ColumnDefinition Width="Auto"/><ColumnDefinition Width="*"/></Grid.ColumnDefinitions>
              <Button x:Name="SectionApplyBtn" Grid.Column="0" MinWidth="260" Height="42" Padding="26,0" FontSize="13" FontWeight="Black" Foreground="#2ed573" Background="#0b2a18" BorderBrush="#2ed573" BorderThickness="1" Content="▶  APPLY SELECTED TWEAKS"/>
              <StackPanel Grid.Column="1" Margin="18,0,0,0" VerticalAlignment="Center">
                <TextBlock x:Name="ApplyProgressText" Text="Selections persist across all sections. Check tweaks anywhere, then apply." Foreground="#7ca7a2" FontSize="12" TextWrapping="Wrap"/>
                <ProgressBar x:Name="ApplyProgressBar" Height="6" Minimum="0" Maximum="100" Value="0" Margin="0,8,0,0" Foreground="#00ffe7" Background="#061016" BorderBrush="#17313d" BorderThickness="1"/>
              </StackPanel>
            </Grid>
          </Border>
        </Grid>
        ...
        <Border x:Name="FooterBorder" Grid.Row="2" Grid.Column="1" Margin="16,0,16,12" CornerRadius="14" Background="#071018" BorderBrush="#f9c74f" BorderThickness="1.1">
      <Grid Margin="22,0">
        ...
        <StackPanel Orientation="Horizontal" VerticalAlignment="Center" HorizontalAlignment="Center">
          <TextBlock Text="&#xE72E;" FontFamily="Segoe MDL2 Assets" FontSize="22" Foreground="#2ed573" Margin="0,0,12,0" VerticalAlignment="Center"/>
          <TextBlock Text="SYSTEM OPTIMIZED" Foreground="#ffffff" FontSize="13" FontWeight="Bold" VerticalAlignment="Center"/>
        </StackPanel>
        <StackPanel Grid.Column="1" Orientation="Horizontal" VerticalAlignment="Center" HorizontalAlignment="Center">
          <TextBlock Text="&#xE945;" FontFamily="Segoe MDL2 Assets" FontSize="22" Foreground="#ffaa2b" Margin="0,0,12,0" VerticalAlignment="Center"/>
          <TextBlock Text="POWER MODE: " Foreground="#ffffff" FontSize="13" FontWeight="Bold" VerticalAlignment="Center"/>
          <TextBlock Text="ULTIMATE PERFORMANCE" Foreground="#b54cff" FontSize="13" FontWeight="Bold" VerticalAlignment="Center"/>
        </StackPanel>
        <StackPanel Grid.Column="2" Orientation="Horizontal" VerticalAlignment="Center" HorizontalAlignment="Center">
          <TextBlock Text="&#xE121;" FontFamily="Segoe MDL2 Assets" FontSize="21" Foreground="#ffffff" Margin="0,0,12,0" VerticalAlignment="Center"/>
          <TextBlock Text="LAST UPDATED: " Foreground="#ffffff" FontSize="13" FontWeight="Bold" VerticalAlignment="Center"/>
          <TextBlock x:Name="LastUpdatedText" Text="--" Foreground="#00ffe7" FontSize="13" FontWeight="Bold" VerticalAlignment="Center"/>
        </StackPanel>
        <Border Grid.Column="3" CornerRadius="10" BorderBrush="#00a7c8" BorderThickness="1" Padding="16,7" HorizontalAlignment="Center" VerticalAlignment="Center"><TextBlock Text="v62.0.0" Foreground="#00ffe7" FontSize="13" FontWeight="Black"/></Border>
      </Grid>
    </Border>
...
$SectionApplyBar     = & $find 'SectionApplyBar'
$ApplyProgressBar   = & $find 'ApplyProgressBar'
$ApplyProgressText  = & $find 'ApplyProgressText'
$SectionApplyBtn     = & $find 'SectionApplyBtn'
if ($SectionApplyBtn) { $SectionApplyBtn.Add_Click({ Invoke-ApplySelected }) }
...
function Set-UltraActionButton {
    ...
    # v57 alignment fix: let the Grid/UniformGrid control width/height.
    # Fixed Width/Height here was overriding the v47 layout and clipping icons/text.
    ...
    $Button.Margin = '5,0'
    $Button.Padding = '6,4'
    $Button.Background = Get-ArgbBrush '#07131c' 0.98
...
function Apply-UltraActionButtons {
    ...
    Set-UltraActionButton (& $find 'OptimizeBtn')      ([string][char]0xE945) 'AUTO OPTIMIZER' '#ffaa2b' $true
...
$script:SectionOrder = @(
    ...
    [pscustomobject]@{ Key='smart';    Title='Smart Engine';     Icon=[char]0xE9D2; IconColor='#2ed573'; Desc='Safe Atom OS, Winaero, and WPD-inspired profiles: privacy, responsiveness, scheduler, startup, and network stability without duplicate/conflicting registry ownership.' },
...
function Update-SelectedCount {
    ...
    # Also count pending selections for sections not currently visible
    if ($script:PendingSelections) { $count += $script:PendingSelections.Count }
...
function Set-TweakApplyingVisualState([string]$id) {
    if (-not $script:CheckboxMap.ContainsKey($id)) { return }
    $entry = $script:CheckboxMap[$id]
    $entry.CheckBox.IsEnabled  = $false
    $entry.CheckBox.Content    = 'Applying...'
    $entry.CheckBox.Foreground = Get-ColorBrush '#ffaa2b'
    $entry.Status.Text         = 'APPLYING'
    $entry.Status.Foreground   = Get-ColorBrush '#ffaa2b'
    $entry.Card.BorderBrush    = Get-ArgbBrush '#ffaa2b' 0.60
    $entry.Card.Background     = Get-ArgbBrush '#ffaa2b' 0.07
    try { [System.Windows.Threading.Dispatcher]::CurrentDispatcher.Invoke([Action]{}, [System.Windows.Threading.DispatcherPriority]::Background) } catch {}
}
...
function Set-TweakFailedVisualState([string]$id) {
    if (-not $script:CheckboxMap.ContainsKey($id)) { return }
    $entry = $script:CheckboxMap[$id]
    $entry.CheckBox.IsEnabled  = $true
    $entry.CheckBox.Content    = 'Retry'
    $entry.CheckBox.Foreground = Get-ColorBrush '#ff4757'
    $entry.Status.Text         = 'FAILED'
    $entry.Status.Foreground   = Get-ColorBrush '#ff4757'
    $entry.Card.BorderBrush    = Get-ArgbBrush '#ff4757' 0.55
    $entry.Card.Background     = Get-ArgbBrush '#ff4757' 0.07
}
...
function Show-UiSettings {
    ...
    $aTitle.Text = 'LAK TWEAKS GOD  —  Version 62'
...
function Set-HomeDashboardVisible([bool]$visible) {
    if ($visible) {
        ...
        $HomeRowDef.Height = [Windows.GridLength]::new(216)
    } else {
...
function Invoke-ApplySelected {
    Ensure-Admin
    # Collect from visible cards
    $selectedIds = [System.Collections.Generic.List[string]]::new()
    foreach ($item in $script:Catalog) {
        if ($script:CheckboxMap.ContainsKey($item.Id) -and $script:CheckboxMap[$item.Id].CheckBox.IsChecked) {
            [void]$selectedIds.Add($item.Id)
        }
    }
    # Also include pending selections (tweaks in sections not currently visible)
    if ($script:PendingSelections) {
        foreach ($id in $script:PendingSelections.Keys) {
            if (-not $selectedIds.Contains($id)) { [void]$selectedIds.Add($id) }
        }
    }
    $selected = @($script:Catalog | Where-Object { $selectedIds -contains $_.Id })
    if ($selected.Count -eq 0) {
        [System.Windows.MessageBox]::Show('No tweaks selected. Go to a section and check the tweaks you want to apply.', 'LakTweaks', 'OK', 'Information') | Out-Null
        return
    }
    ...
    $r = [System.Windows.MessageBox]::Show(
        ...
        'LakTweaks v62','YesNo','Warning')
    if ($r -ne 'Yes') { return }

    try {
        Backup-LakQuickRegistry
        New-LakStateSnapshot $selected | Out-Null
        Set-ApplyProgress 0 $($selected.Count) 'Starting apply run'
        ...
        if ($result.Failed.Count -gt 0) {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count), failed $($result.Failed.Count). Check the log, then use RETRY FAILED.", 'LakTweaks v62', 'OK', 'Warning') | Out-Null
        } else {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count) tweak(s) successfully.`nReboot your PC for full effect.", 'LakTweaks v62', 'OK', 'Information') | Out-Null
        }
    } catch {
...
function Show-Section([string]$key) {
    ...
    if ($key -eq 'home') {
        ...
        if ($SectionApplyBar) { $SectionApplyBar.Visibility = 'Collapsed' }
        $SectionTitle.Text   = ''
        $SectionDesc.Text    = ''
        $CardPanel.ItemWidth = [double]::NaN
        # NEVER reset CheckboxMap - selections persist across navigation
        Update-SelectedCount
        return
    }
    ...
    # Build cards - restore prior checkbox state from persistent CheckboxMap
    $priorSelections = @{}
    foreach ($kv in $script:CheckboxMap.GetEnumerator()) {
        $priorSelections[$kv.Key] = $kv.Value.CheckBox.IsChecked
    }
    # Only clear entries for this section (re-create cards) but keep other sections
    $sectionIds = @($script:Catalog | Where-Object CategoryId -eq $key | Select-Object -ExpandProperty Id)
    foreach ($id in $sectionIds) { $script:CheckboxMap.Remove($id) }

    $items = $script:Catalog | Where-Object CategoryId -eq $key
    foreach ($item in $items) {
        $card = New-Card $item
        $CardPanel.Children.Add($card) | Out-Null
        # Restore previous selection state if this tweak was checked before
        if ($priorSelections.ContainsKey($item.Id) -and $priorSelections[$item.Id] -eq $true) {
            if ($script:CheckboxMap.ContainsKey($item.Id) -and $script:CheckboxMap[$item.Id].CheckBox.IsEnabled) {
                $script:CheckboxMap[$item.Id].CheckBox.IsChecked = $true
            }
        }
    }

    if ($SectionApplyBar) { $SectionApplyBar.Visibility = 'Visible' }
...
foreach ($sec in $script:SectionOrder) {
    ...
    $ico.FontSize = 18; $ico.Foreground = Get-ColorBrush $sec.IconColor; $ico.Width = 34; $ico.VerticalAlignment = 'Center'
    $lbl = [Windows.Controls.TextBlock]::new()
    $lbl.Text = $sec.Title; $lbl.FontSize = 12; $lbl.FontWeight = 'SemiBold'; $lbl.TextWrapping = 'NoWrap'; $lbl.TextTrimming = 'None'; $lbl.Foreground = [Windows.Media.Brushes]::White; $lbl.VerticalAlignment = 'Center'
...
(& $find 'ClearLogBtn').Add_Click({
    if ($null -ne $LogBox) { $LogBox.Text = 'LakTweaks v62 log cleared.' }
...
(& $find 'RetryFailedBtn').Add_Click({
    ...
    Update-SelectedCount
    Write-Log ("Retrying " + $script:LastFailedTweaks.Count + " failed tweak(s) now...") 'INFO'; Invoke-ApplySelected
})

(& $find 'OptimizeBtn').Add_Click({ Select-AutoOptimizerPreset })
...
(& $find 'RestoreBackupBtn').Add_Click({ Restore-LakLastSnapshot })
(& $find 'RamCleanBtn').Add_Click({ Clean-LakRamLite })

(& $find 'ApplyBtn').Add_Click({ Invoke-ApplySelected })


Apply-UltraActionButtons
Apply-Theme
Initialize-FastPerformanceCounters
Refresh-LiveStatsCache
Update-PerformancePanel
Start-LakStatsTimer
...
Write-Log 'v62 PRO+: undo snapshots, verify mode, progress engine, conflict guard, uploaded USB/GPU/power tweaks added.' 'OK'
Show-Section 'home'
Update-SelectedCount
$Window.Add_Closed({ if ($script:StatsTimer) { try { $script:StatsTimer.Stop() } catch {} }; if ($script:LowTimerActive -and ('WinMM.NativeMethods' -as [type])) { try { [void][WinMM.NativeMethods]::timeEndPeriod(1) } catch {} } })
