...
function New-LakRestorePoint {
    try {
        Write-Log 'Creating restore point: LakTweaks v59 Before Apply...' 'INFO'
        Checkpoint-Computer -Description 'LakTweaks v59 Before Apply' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
...
function Backup-LakFullRegistry {
    try {
        ...
        [System.Windows.MessageBox]::Show('Full registry backup saved to:`n' + $dir, 'LakTweaks v59', 'OK', 'Information') | Out-Null
...
function Restore-LakFullRegistry {
    try {
        ...
        $r = [System.Windows.MessageBox]::Show('Restore the saved full registry backup now? You should reboot after restoring.', 'LakTweaks v59', 'YesNo', 'Warning')
        if ($r -ne 'Yes') { return }
...
function Select-AutoOptimizerPreset {
    ...
    $denyPattern    = 'extreme|strip|defender|wupdate|tdr|spectre|cstates|trusted|eventlog|samss|full_service|oneclick_full|disable_windows_update|windows_update|hidsvc|hidserv|nlasvc|lanman|gameinput|fortnite_settings_note'
...
function Invoke-TweakCommands([object[]]$selected) {
    ...
    foreach($item in $selected) {
        ...
        try {
            ...
            # JSON stores registry paths with escaped backslashes. Normalize double slashes before cmd.exe sees them.
            ...
            # A lot of normal tweaks call sc stop/taskkill on things that may not exist. If the tweak prints [OK], do not mark it failed just because cmd's last non-echo command returned nonzero.
            $okMarker = ($output -match '\[OK\]')
            $hardFail = ($output -match '(?i)Access is denied|Invalid syntax|The syntax of the command is incorrect')
  ...
  <Grid>
      ...
      <Grid Margin="22,0,22,0">
        ...
        <StackPanel Orientation="Vertical" VerticalAlignment="Center">
          <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
            ...
            <TextBlock Text=" v57" FontSize="25" FontWeight="Black" Foreground="#00ffe7" Margin="14,0,0,0"/>
        ...
        <Grid>
          ...
          <Border x:Name="SectionApplyBar" Grid.Row="2" Visibility="Collapsed" Margin="0,14,0,0" Padding="14,10" CornerRadius="12" Background="#071f14" BorderBrush="#2ed573" BorderThickness="1.1">
            <Grid>
              ...
              <TextBlock Grid.Column="1" Text="Selections persist across all sections. Check tweaks anywhere, then apply." VerticalAlignment="Center" Foreground="#7ca7a2" FontSize="12" Margin="18,0,0,0" TextWrapping="Wrap"/>
            </Grid>
          </Border>
        </Grid>
      </Border>
    ...
    <Border x:Name="FooterBorder" Grid.Row="2" Grid.Column="1" Margin="16,0,16,12" CornerRadius="14" Background="#071018" BorderBrush="#00a7c8" BorderThickness="1.1">
      <Grid Margin="22,0">
        ...
        <Border Grid.Column="3" CornerRadius="10" BorderBrush="#00a7c8" BorderThickness="1" Padding="16,7" HorizontalAlignment="Center" VerticalAlignment="Center"><TextBlock Text="v59.0.0" Foreground="#00ffe7" FontSize="13" FontWeight="Black"/></Border>
      </Grid>
    </Border>
...
function Invoke-ApplySelected {
    ...
    try {
        ...
        if ($result.Failed.Count -gt 0) {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count), failed $($result.Failed.Count). Check the log, then use RETRY FAILED.", 'LakTweaks v59', 'OK', 'Warning') | Out-Null
        } else {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count) tweak(s) successfully.`nReboot your PC for full effect.", 'LakTweaks v59', 'OK', 'Information') | Out-Null
        }
    } catch {
...
(& $find 'ClearLogBtn').Add_Click({
    if ($null -ne $LogBox) { $LogBox.Text = 'LakTweaks v59 log cleared.' }
...
Write-Log 'v59 APPLY FIX: apply button clipping fixed, command runner normalized, success/fail tracking improved.' 'OK'
Show-Section 'home'
Update-SelectedCount
