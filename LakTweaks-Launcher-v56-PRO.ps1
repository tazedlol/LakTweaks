...
function New-LakRestorePoint {
    try {
        Write-Log 'Creating restore point: LakTweaks v56 Before Apply...' 'INFO'
        Checkpoint-Computer -Description 'LakTweaks v56 Before Apply' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
...
function Initialize-FastPerformanceCounters {
    # v56 PRO: live stats without UI lag.
...
function Backup-LakFullRegistry {
    try {
        ...
        [System.Windows.MessageBox]::Show('Full registry backup saved to:`n' + $dir, 'LakTweaks v56 PRO', 'OK', 'Information') | Out-Null
...
function Restore-LakFullRegistry {
    try {
        ...
        $r = [System.Windows.MessageBox]::Show('Restore the saved full registry backup now? You should reboot after restoring.', 'LakTweaks v56 PRO', 'YesNo', 'Warning')
        if ($r -ne 'Yes') { return }
...
function Select-AutoOptimizerPreset {
    ...
    $baseCategories = @('smart','input','network','power','gpu','privacy','registry','debloat')
    $denyPattern = 'extreme|strip|defender|wupdate|tdr|spectre|cstates|trusted|eventlog|samss|full_service|oneclick_full|disable_windows_update|windows_update|hidsvc|hidserv|nlasvc|lanman|gameinput'

    $ids = @($script:Catalog | Where-Object {
        ($_.CategoryId -in $baseCategories) -and ($_.Id -notmatch $denyPattern) -and ($_.Title -notmatch 'Extreme|OneClick Full|Defender|Windows Update')
    } | Select-Object -ExpandProperty Id)

    if ($hw.GPU -notmatch 'NVIDIA') {
        $ids = @($ids | Where-Object { $_ -notmatch 'nvidia|msi_latency|shader|nv' })
        Write-Log 'Auto Optimizer: NVIDIA-specific tweaks skipped because NVIDIA GPU was not detected.' 'INFO'
    }
    if ($hw.Network -match 'Wi-?Fi|Wireless') {
        $ids = @($ids | Where-Object { $_ -notmatch 'nic_interrupt|green_ethernet|eee' })
        Write-Log 'Auto Optimizer: Ethernet-only NIC latency tweaks skipped because Wi-Fi/wireless was detected.' 'INFO'
    }

    foreach($id in ($ids | Select-Object -Unique)) { Select-TweakById $id }
    Update-SelectedCount
    Write-Log ('Auto Optimizer queued ' + (($ids | Select-Object -Unique).Count) + ' conflict-safe performance tweaks. Review the selected cards, then click APPLY SELECTED.') 'PRESET'
}
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
  ...
  <Grid>
      ...
      <Grid Margin="22,0,22,0">
        ...
        <StackPanel Orientation="Vertical" VerticalAlignment="Center">
          <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
            ...
            <TextBlock Text=" v56" FontSize="25" FontWeight="Black" Foreground="#00ffe7" Margin="14,0,0,0"/>
    ...
    <Grid x:Name="MainContentGrid" Grid.Row="1" Grid.Column="1" Margin="16,16,16,14">
      <Grid.RowDefinitions><RowDefinition x:Name="HomeRowDef" Height="205"/><RowDefinition x:Name="ContentRowDef" Height="*"/></Grid.RowDefinitions>
      ...
      <Border x:Name="HomeDashboardBorder" Grid.Row="0" CornerRadius="18" Background="#071018" BorderBrush="#183542" BorderThickness="1.2" Padding="14,10">
        <Grid>
          ...
          <UniformGrid Grid.Row="0" Rows="1" Columns="9" HorizontalAlignment="Stretch" VerticalAlignment="Stretch">
            <Button x:Name="SafePresetBtn"    Margin="3,0" Height="58" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="CompPresetBtn"    Margin="3,0" Height="58" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="ExtremePresetBtn" Margin="3,0" Height="58" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="SelectAllBtn"     Margin="3,0" Height="58" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="ResetAllBtn"      Margin="3,0" Height="58" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="ExportBatBtn"     Margin="3,0" Height="58" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="RestorePointBtn"  Margin="3,0" Height="58" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="RetryFailedBtn"   Margin="3,0" Height="58" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="ClearLogBtn"      Margin="3,0" Height="58" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
          </UniformGrid>
          ...
          <Grid Grid.Row="2" HorizontalAlignment="Stretch" VerticalAlignment="Stretch">
            ...
            <Button x:Name="OptimizeBtn"      Grid.Column="0" Margin="3,0" Height="48" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="LowTimerBtn"      Grid.Column="1" Margin="3,0" Height="48" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="SmartNetBtn"      Grid.Column="2" Margin="3,0" Height="48" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="FullBackupBtn"    Grid.Column="3" Margin="3,0" Height="48" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="RestoreBackupBtn" Grid.Column="4" Margin="3,0" Height="48" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="RamCleanBtn"      Grid.Column="5" Margin="3,0" Height="48" FontSize="9" FontWeight="Bold" Background="#07131c" Foreground="#ffffff"/>
            <Button x:Name="ApplyBtn"         Grid.Column="6" Margin="3,0" Height="48" FontSize="9" FontWeight="Black" Background="#071f14" Foreground="#00ffe7"/>
            <Border Grid.Column="7" Margin="4,0,0,0" CornerRadius="10" Background="#090f15" BorderBrush="#243b45" BorderThickness="1" Padding="4,2">
    ...
    <Border x:Name="FooterBorder" Grid.Row="2" Grid.Column="1" Margin="16,0,16,12" CornerRadius="14" Background="#071018" BorderBrush="#00a7c8" BorderThickness="1.1">
      <Grid Margin="22,0">
        ...
        <Border Grid.Column="3" CornerRadius="10" BorderBrush="#00a7c8" BorderThickness="1" Padding="16,7" HorizontalAlignment="Center" VerticalAlignment="Center"><TextBlock Text="v56.0.0" Foreground="#00ffe7" FontSize="13" FontWeight="Black"/></Border>
      </Grid>
    </Border>
...
function Set-UltraActionButton {
    ...
    $Button.Width = if ($Label -eq 'APPLY SELECTED') { 210 } elseif ($Wide) { 170 } else { 124 }
    $Button.Height = if ($Wide -or $Label -eq 'APPLY SELECTED') { 58 } else { 76 }
    $Button.Padding = '8,6'
    $Button.HorizontalContentAlignment = 'Center'
    $Button.VerticalContentAlignment = 'Center'
    $stack = [Windows.Controls.StackPanel]::new()
    $stack.Orientation = 'Vertical'; $stack.HorizontalAlignment = 'Center'; $stack.VerticalAlignment = 'Center'
    $iconBlock = [Windows.Controls.TextBlock]::new()
    $iconBlock.Text = $Icon
    $iconBlock.FontFamily = [Windows.Media.FontFamily]::new('Segoe MDL2 Assets')
    $iconBlock.FontSize = if ($Wide -or $Label -eq 'APPLY SELECTED') { 21 } else { 23 }
    $iconBlock.Foreground = Get-ColorBrush $Color
    $iconBlock.HorizontalAlignment = 'Center'; $iconBlock.VerticalAlignment = 'Center'
    $iconBlock.Margin = if ($Wide -or $Label -eq 'APPLY SELECTED') { '0,0,0,0' } else { '0,0,0,6' }
    $textBlock = [Windows.Controls.TextBlock]::new()
    $textBlock.Text = $Label
    $textBlock.FontSize = if ($Wide -or $Label -eq 'APPLY SELECTED') { 12 } else { 10 }
    ...
    $textBlock.HorizontalAlignment = 'Center'; $textBlock.TextAlignment = 'Center'; $textBlock.TextWrapping = 'NoWrap'; $textBlock.TextTrimming = 'None'
    if ($Wide -or $Label -eq 'APPLY SELECTED') { $stack.Orientation = 'Horizontal'; $iconBlock.Margin = '0,0,10,0'; $textBlock.VerticalAlignment = 'Center' }
...
function Apply-UltraActionButtons {
    ...
    Set-UltraActionButton (& $find 'OptimizeBtn')      ([string][char]0xE945) 'AUTO OPTIMIZER' '#ffaa2b'
...
function Set-HomeDashboardVisible([bool]$visible) {
    if ($visible) {
        ...
        $HomeRowDef.Height = [Windows.GridLength]::new(205)
    } else {
...
(& $find 'ClearLogBtn').Add_Click({
    if ($null -ne $LogBox) { $LogBox.Text = 'LakTweaks v56 log cleared.' }
...
(& $find 'ApplyBtn').Add_Click({
    ...
    $r = [System.Windows.MessageBox]::Show(
        ...
        'LakTweaks v56 PRO','YesNo','Warning')
    if ($r -ne 'Yes') { return }
    ...
    try {
        ...
        if ($result.Failed.Count -gt 0) {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count), failed $($result.Failed.Count). Check the log panel, then use RETRY FAILED if needed.", 'LakTweaks v56 PRO', 'OK', 'Warning') | Out-Null
        } else {
            [System.Windows.MessageBox]::Show("Applied $($result.Succeeded.Count) tweak(s) successfully.`nReboot your PC for full effect.", 'LakTweaks v56 PRO', 'OK', 'Information') | Out-Null
        }
    } catch {
